...
- **kill**: `{"action":"kill","name":"designer"}` — closes its terminal (shell + claude + respawn loop). **Writing this request can itself need the operator's approval.** The permission check judges what a request *does*, not that it is a file: a `kill` of a live session was held for the operator's approval in the session writing it (2026-09-23), while a `resume` naming a session that never existed was not (2026-09-25; the surface dead-lettered it, as designed). So a brief that plans a kill says so, and runs where the operator can approve it; an unattended routine should not count on its kills landing. An operator who wants inbox writes to go through without asking can add a permission rule allowing writes to `~/.aios/spawn-inbox/` — their call, never a default, because it also lets a session close other sessions unasked.
...
> ⚠️ **Contain a headless worker with the tools that exist, not the ones you allow:** `claude -p '<prompt>' --tools "<only what it needs>" --setting-sources user,project --strict-mcp-config --permission-mode default`. `--allowedTools` is additive to every rule in the settings that load, including the *"allow always"* clicks accumulated in `.claude/settings.local.json` — `--tools` sets what exists and `--setting-sources user,project` drops that layer. Full measurement: `MODEL-ROUTING.md` § Verify an id before you trust it.
>
> ⚠️ **And the explicit permission mode is still required.** Measured on a machine with `permissions.defaultMode: "auto"` in `settings.json`: a `claude -p` passing an allowlist naming a nonexistent tool, plus `--strict-mcp-config`, still performed `Bash` and `Write` calls — no error, no prompt, `permission_denials: []`. **A machine-level auto mode silently outranks the flag.** Pass `--permission-mode default` (or `plan`) beside it and the same call is blocked.
>
