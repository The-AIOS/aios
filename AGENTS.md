...
**Context loading is how a session demonstrates reinforced learning** — the only moment where what the operator and Claude built together reaches this session or does not, and nothing about the output will say which. The measure is not how much you read; it is whether what you read **fits** what you were asked to do.

1. **The floor, always, whatever the task — one command:** `UV_CACHE_DIR="${TMPDIR:-/tmp}/uv-cache" uv run ~/aios/hooks/context-floor.py` — `uv` because on Windows `python3` is a Store alias that runs nothing, and `UV_CACHE_DIR` because a sandboxed shell cannot write `~/.cache/uv`; either way the floor would emit nothing, which is indistinguishable from one that ran. It emits both `_index.md`, **every heading in both folders**, **the newest 5 `###` entries of every `observed/` file in full** (newest by the date each entry carries, not by file position — measured, the tail is the newest in only 5 of 8 files), except a **rule library** like `antifragile.md`, which emits its meta-pattern index instead: an entry from four months ago binds as hard as one from this week, and **`INTENT.md`** — which is at the floor because it governs what you may *do*, a different question from whether your output sounds like them, and one that binds the mechanical worker hardest. Absent the hook, glob it by hand (`declared/*.md`, `observed/*.md`) — never a list of filenames, since both folders vary per vault and a named list silently skips what it does not mention. **The floor carries learned content, not only titles:** every close-session appends to `observed/`, and if the next session never reads what was appended the loop does not close. It reads the *tail* because `declared/` is restated in place while `observed/` accumulates — and the tail is bounded, so it costs the same on a vault ten times the size.
2. **Above the floor there are no tiers — only fit.** Intelligence is the right information at the right time, not the largest pile you can afford. **Depth is yours to set, per file, without asking** — the floor's five recent entries are a starting position, not an allowance; read past them, or read one `declared/` file rather than the folder. **Later beats speculatively earlier**: reaching mid-task is the normal mode. Reading everything is the *degenerate* case, not the top of a ladder. The floor is the minimum. **Each rung includes the one below it** — climbing adds to the floor, never replaces it. **If the task touches a venture, read that venture's folder** from `context/ventures/`: it is partitioned rather than global, so the floor lists the ventures and you open the one that applies. `about_business.md` is a ~1:100 summary and does not substitute. **Every file in both folders, read whole, is the maximum — and it is reachable**: right when the whole context is cheap (`hooks/context-rungs.py` says so) and right at any size when **the task is the context itself** — synthesising across their history, auditing or compacting the observed files, answering *as* them. An index cannot serve those; the corpus is the input. The question that settles most of what lies between: *will what I produce be read as the operator's own words, or act on their behalf?* **Operator-facing is broader than ghostwriting** — a message, post, note, release note, README line, repo description, email or reply all land as theirs. The test is *will a human read this as coming from them*, not *was I asked to imitate them*.
...
- **State intent, then act.** For non-routine action, say it in one line — *"I intend to [action] because [reasoning]."* — then do it and report. Stop only before an irreversible or outward-facing step, a decision that is the operator's, or when you cannot continue. Not for routine reads/edits or explicit requests.
