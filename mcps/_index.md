...
So a failed MCP is almost always a **local state issue** (stale token, missing `.venv`, double-nested folder, port held by a stale process), NOT a lost-key issue. Start debugging locally — check `claude mcp list`, the MCP's `.venv`, and `mcp_server_debug.log` — before touching any cloud console. See `google-workspace-mcp/TROUBLESHOOTING.md` for the full local-first recovery flow.
