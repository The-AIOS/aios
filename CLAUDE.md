...
4. After greeting, **load context — and *do* it, as the first tool calls of the session, rather than describing it.** Two folders, and what decides how you read each is their **size** — measured in the vault in front of you, never assumed and never inferred from how long the vault has existed. A heavily-used vault runs `observed/` at several times `declared/`; a lightly-used one of the same age is small in both. **Size is the variable; age only correlates with it.**
   ...
   **b. Then judge the read against the task — a RANGE, not a switch. Each rung *includes* the one below it**, so climbing adds to the floor and never replaces it. Decide how much more to open, between two reachable bounds:

   - **Minimum — the floor you already have.** Work whose correctness is checkable without knowing the operator: code, tests, file operations, data, mechanical sweeps. **The map is enough**; open an `observed/` entry the moment a title tells you it applies.
   - **Maximum — every file in `declared/`, `observed/` *and* every venture, read whole.** Not a failure state. Right when (i) the whole context is *cheap* — run `python3 ~/aios/hooks/context-rungs.py` and do what its verdict says; or (ii) **the task is the context itself**: synthesising across their history, auditing or compacting the observed files, deriving a pattern that only appears across many of them, answering *as* them. An index cannot serve those — the corpus is the input.
   - **The question that settles most of what lies between:** *will what I produce be read as the operator's own words, or act on their behalf?* **Yes → the floor plus all of `vault/00 - notes/context/declared/`.** (`INTENT.md` you already have — it is at the floor.) Voice is the one thing whose absence you **cannot detect in your own output**: under-read `observed/` and the gap shows as work visibly missing something; under-read `declared/` and the work comes back fluent, correct, and **not theirs**, with nothing in it looking wrong. Only one of the two failures announces itself.
   - **Does the task touch a venture? Read that venture's folder.** `context/ventures/` is a **third** access pattern because it is *partitioned* where the other two are global: any `observed/` entry might bear on any task, but venture context is scoped, and which venture applies is usually plain from the task. So the floor lists them and you open the one that applies. **`about_business.md` does not substitute** — measured on a live vault it is ~1,000 words summarising over 100,000, and § IV says plainly that the detail lives in each venture's own files. Reading the summary and believing you have venture context is mistaking the index for the territory. Run `hooks/context-rungs.py` for what each venture costs; one venture is typically a fraction of the folder.
   - **Genuinely hard to place?** Load the **`right-context`** skill — the full ladder, the tells that say you are already guessing, and the mid-task climb.
