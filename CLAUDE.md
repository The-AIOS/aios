...
>
> The auto-mode permission classifier reads `spawn <name> …` as *"launch an autonomous agent"* and auto-denies it silently (a red dot, no prompt), and the keystroke path it would fall back to leaks synthetic keystrokes. So `spawn`/`spawn-kill` are for **humans** at a terminal and for a surface's own use. As an agent:
>
> - **Need a worker → write a request file, never run `spawn`.** If `~/.aios/spawn-inbox/` exists, `Write` `~/.aios/spawn-inbox/<name>.json` = `{"name":"<kebab>","task":"<first prompt>"}`, plus optional `"model":"<id>"` **or** `"tier":"mechanical"|"judgment"` (by cognitive load — the `orchestration-ladder` skill), plus `"surface"`. The running surface — AIOS Glass in the IDE (`vscode.createTerminal`) or the AIOS App — fulfils it natively: no keystrokes, no gate. Writing a file is not gated.
...
> - **No surface running** (`~/.aios/spawn-inbox/` absent) → do **not** call `spawn`. Hand it to the operator — the Glass **"Spawn a session"** button, or `spawn <name> "<task>"` in a fresh terminal — with the exact name + task so it's one paste. **The tell you got it wrong:** a red-dot *"tool use was rejected"*, or a `spawn` that opens no terminal.
...
**Killing a spawned worker:** `spawn-kill {name}` — atomic process-group kill (respawn loop + claude + descendants in one step), then closes the Terminal window (macOS). Not SIGTERM (the respawn loop catches it and re-launches) and not Cmd+W (Terminal's modal, and it risks orphaning claude to launchd).
