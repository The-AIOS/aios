...
4. After greeting, **load context — and *do* it, as the first tool calls of the session, rather than describing it.** Two folders, and what decides how you read each is their **size** — measured in the vault in front of you, never assumed and never inferred from how long the vault has existed. A heavily-used vault runs `observed/` at several times `declared/`; a lightly-used one of the same age is small in both. **Size is the variable; age only correlates with it.**
   ...
   **a. The floor, always, whatever the task — one command.**
   ...
   It emits, in one call: both `_index.md`, **every heading in both folders**, and **the last 5 `###` entries of every `observed/` file, in full**. Absent the hook, glob it by hand — `grep -h '^#' "…/declared/"*.md`, `grep -h '^###' "…/observed/"*.md`, then the tail of each observed file — but prefer the hook, because a floor assembled by hand is done partially or skipped and the difference is invisible afterwards.
   ...
   **b. Then judge the read against the task — a RANGE, not a switch.** You hold the floor now. Decide how much more to open, between two reachable bounds:
