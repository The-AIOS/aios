...
4. After greeting, **load context — and *do* it, as the first tool calls of the session, rather than describing it.** Two folders, and what decides how you read each is their **size** — measured in the vault in front of you, never assumed and never inferred from how long the vault has existed. A heavily-used vault runs `observed/` at several times `declared/`; a lightly-used one of the same age is small in both. **Size is the variable; age only correlates with it.**
   ...
   **a. The map, always, whatever the task.** List `vault/00 - notes/context/declared/` and `…/observed/`, read both `_index.md`, then read **every heading in both folders** — `grep -h '^#' "…/declared/"*.md` and `grep -h '^###' "…/observed/"*.md`. A few hundred lines total. This is your floor *and* your index: `observed/` is written as discrete entries, so a title like `110. Empty and no-op inputs resolve to something REAL` tells you the lesson exists and exactly when to open it, and `declared/`'s headings tell you what the operator has declared about themselves without your reading it yet. `antifragile.md` most of all — § Session Start calls it the file to *scan for relevant rules before executing commands*.

   > **Glob the folders — never work from a list of filenames, including the one you are reading.** Both folders vary from vault to vault: the framework ships five declared files and nine observed ones, and operators add their own (`psychometric-profile`, `coding-practices`, a venture file, anything). A named list is a second implementation of the folder's contents, and it fails the way every hardcoded list in this repo has failed — silently, by skipping what it does not mention. Measured on one live vault, a floor naming four observed files by name missed **103 entry titles across five other files**, among them the venture map and the identity synthesis. The `*.md` glob costs the same to write and cannot go stale.

   **b. Then judge the read against the task — this is a RANGE, not a switch.** You now hold the map, so you know what exists and roughly what it costs. Decide how much of it to *open*, between two real bounds. Both bounds are reachable, and most sessions land between them:

   - **Minimum — the map you already have.** Work whose correctness is checkable without knowing the operator: code, tests, file operations, data, mechanical sweeps. **The map is enough**; open an `observed/` entry the moment one of its titles tells you it applies.
   - **Maximum — every file in both folders, read whole.** Not a failure state, and not reserved for small vaults. It is simply right when either (i) the whole context is *cheap* — run `python3 ~/aios/hooks/context-rungs.py`, and if its verdict says read all of it, read all of it, because on a light vault the entire context costs less than this page; or (ii) **the task is about the context itself** — synthesising across the operator's history, auditing or compacting their observed files, deriving a pattern that only appears across many of them, or answering *as* them from everything they have ever said. Those tasks are not served by an index; the whole corpus is the input.
   - **The question that settles most of what lies between:** *will what I produce be read as the operator's own words, or act on their behalf?* **Yes → all of `vault/00 - notes/context/declared/` and `INTENT.md`.** It is the **small** folder — who they are, their voice, how they decide, their ventures — and voice is the one thing whose absence you **cannot detect in your own output**. That asymmetry is why this read is worth paying for: under-read `observed/` and the gap shows up as work visibly missing something, or a question you know to ask; under-read `declared/` and the work comes back fluent, correct, and **not theirs**, with nothing in it looking wrong. Only one of the two failures announces itself.
   - **Genuinely hard to place?** Load the **`right-context`** skill. It carries the full ladder, the tells that say you are already guessing, and the mid-task climb. That is where this judgment lives in full; this is the short form.

   **c. Whatever you opened, the rest stays open to you — that is the point of the map.** Note what `observed/` holds: you have its entry titles, not its bodies. That is a deliberate index-then-open pattern, **not because any of it is off limits.** Open any entry the moment a title or the work points at it — a task touching a person or a venture usually wants `ecosystem.md` or `business.md`; one where you are unsure how they like things done wants `preferences.md`. **Reaching mid-task is expected, not exceptional.** If you catch yourself guessing at how they would phrase something, who someone is, or whether they have already decided this — stop guessing and read.

   **d. Unsure is not a third answer — read more, not less**, for the asymmetry in (b). Over-reading costs tokens on one session; under-reading costs the operator's voice and fails silently.
