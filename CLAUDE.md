...
4. After greeting, **load context — and *do* it, as the first tool calls of the session, rather than describing it.** Two folders, and what decides how you read each is their **size** — measured in the vault in front of you, not assumed. On a mature vault `observed/` runs several times `declared/`; on a fresh clone both are tiny.
   ...
   **a. The map, always, whatever the task.** List `vault/00 - notes/context/declared/` and `…/observed/`, read both `_index.md` (a one-line description per file), then read the **`###` entry titles** of `observed/antifragile.md`, `observed/preferences.md`, `observed/patterns.md` and `observed/growth.md` (`grep '^### '`). A few hundred lines total. This is your floor *and* your index: those four are written as discrete entries, so a title like `110. Empty and no-op inputs resolve to something REAL` tells you the lesson exists and exactly when to open it. `antifragile.md` most of all — § Session Start calls it the file to *scan for relevant rules before executing commands*.
