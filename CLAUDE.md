...
>
> - **You (a Claude session) need a worker → check for the spawn-inbox** (`~/.aios/spawn-inbox/` exists). If present, **write a request file** with the Write tool — `~/.aios/spawn-inbox/<name>.json` = `{"name":"<kebab>","task":"<first prompt>"}`, plus optional `"model":"<id>"` **or** `"tier":"mechanical"|"judgment"` (pick by cognitive load — see the `orchestration-ladder` skill), plus `"surface"` (read the next bullet — it matters for `spawn`). Whichever surface is running fulfils it **natively** — AIOS Glass inside the IDE (`vscode.createTerminal`), or the AIOS App through its own fulfiller — no keystrokes, no gate. This is the *only* way an agent spawns an interactive worker. (Writing a file is benign — the classifier doesn't gate it.)
> - **`spawn` SHOULD carry `surface`, and you can derive it — `send`/`kill` do not need it.** The asymmetry is the whole rule. A **`send`/`kill`** names a session that already exists, so a mis-race self-corrects: a surface that can't reach the target's terminal hands the request back, bounded, and it never returns to a fulfiller that already tried. A **`spawn` cannot self-correct** — the session doesn't exist yet, so there is no host to compare against and nothing to hand back; it opens wherever it landed. Unaddressed, that is whichever surface wins the race, which is how a request written from an IDE session gets its worker opened as an App pane instead. So derive it: each surface announces its process-tree root at `~/.aios/surfaces/<surface>.json` — walk your own process ancestry and **compare pids, never process names** (Glass runs inside whatever IDE the operator uses, so a name list breaks the day they switch editors). The snippet is in the inbox README. **No match is a legitimate answer, not an error** — a session started by the `spawn` wrapper in a plain terminal belongs to neither surface; omit `surface` and let them race. Addressing a `send` is still the fastest path when you already know where the target lives, and the right call for anything time-sensitive.
> - **Never hand-write a field beginning with `_`.** `_claim`, `_tried`, `_releases` and `_undelivered` are written by fulfillers as they work — `_tried` is the list of `surface:pid` ids that have already attempted a request, and pre-setting it in a fresh request makes surfaces skip work they should be doing. A request that arrives with any of them already populated is misread.
...
**Scope the worker's MCP servers (`--profile <name>`).** `spawn --profile {name} {worker} "{task}"` starts the session with **only** the servers declared in `~/.aios/mcp-profiles/{name}.json` (via `--strict-mcp-config`); omit it and nothing changes. Reach for it when a worker has no business holding live Gmail, Drive, Slack or home-automation credentials — a file sweep needs none of them. **Do not reach for it to save tokens:** measured, a profile trims 2-4k off a ~50k startup (2-3% of a full ritual session), because MCP tool definitions load *deferred* — names, not schemas. The real payoff is blast radius, plus one failure mode that is not about size at all: **a server that disconnects mid-session invalidates the whole prompt prefix and pays a full cache rebuild, however small it was.** Don't load what you won't call.

> **Why a flag and not a project-scoped `.mcp.json`.** Claude Code also scopes MCPs per *directory*. That works when workloads live in separate repos; it does nothing on a vault-anchored setup where every session runs from `~/aios`. Measure before choosing: if your sessions share a directory, only a per-invocation flag can separate them.
>
> **The OAuth caveat, which costs an hour if you meet it blind.** An http server that authenticates by OAuth holds its grant under **the name it was authorised as**. Rename it inside a profile and it *loads but cannot be called* — it appears in the server list and every tool answers that it needs authentication. Fix: authorise it once under the new name with `/mcp`, in an interactive terminal (a `-p` run cannot). stdio servers (`npx`/`uvx`) have no such step and work in a profile immediately.
