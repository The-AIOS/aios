...
4. After greeting, **load context — and *do* it, as the first tool calls of the session, rather than describing it.** Two folders, and they are different in kind, which is what decides how you read each:
   ...
   **b. Then one question about your own output — not about the files.** *Will what I produce be read as the operator's own words, or act on their behalf?*
   ...
   - **Yes → also read all of `vault/00 - notes/context/declared/`** (~20k words) **and `INTENT.md`.** That folder is *who they are* — voice, identity, how they decide, their ventures — and it is **prose, not entries, so it does not index: you read it or you guess at it.** Guessing is how work comes back fluent, correct, and not theirs.
   ...
   **c. Either way, the rest stays open to you — that is the point of the map.** `observed/` is ~100k words of what has been learned working with this operator. You are not reading it up front because **titles are a better index into it than prose is**, not because it is off limits. Open any of it the moment a title or the work points at it — a task touching a person or a venture usually wants `ecosystem.md` or `business.md`; one where you are unsure how they like things done wants `preferences.md`. **Reaching mid-task is expected, not exceptional.** If you catch yourself guessing at how they would phrase something, who someone is, or whether they have already decided this — stop guessing and read.

   **d. Unsure is not a third answer — read `declared/`.** The two errors are not symmetric. Over-reading costs tokens on one session. Under-reading costs the operator's voice, and it fails *silently*.

   > **Why this shape.** Reading everything is ~120k words before the first question, which is why, measured across spawned workers on a live vault, the rule was producing anywhere from 0 to 18 files with no floor underneath and nothing reporting the zeroes. But the opposite failure is just as real: a worker sent to read *all* of both folders for a two-sentence task spent 38 tool calls and had still not written the two sentences. So the split is by **kind**: the identity layer is small and unindexable, so it is read whole when the output speaks as them; the learned layer is large and entry-structured, so it is mapped and opened by title. And the decision is made by the **shape of the work, not the identity of the session** — a rule keyed on who is running would make every spawned agent permanently thinner than the primary session, including the ones whose whole job is to sound like the operator.
