...
4. After greeting, **load context — and *do* it, as the first tool calls of the session, rather than describing it.**
   ...
   **b. Above the floor there are no tiers — only fit.** **Intelligence is the right information at the right time, not the largest pile you can afford.** There is nothing to qualify for and no level to reach: open what the work points at, to the depth it needs, when it points there.
   ...
   - **Two depths worth naming, as costs rather than permissions.** **All of `vault/00 - notes/context/declared/`** when your output will be read as the operator's own words or act on their behalf. **Operator-facing is far broader than ghostwriting, and this is where the question gets misread.** If a human will read your prose and it carries their name or their team's — a message, a post, a note, a release note, a README line, a repo description, an email, a reply, a doc, a commit body that others read — it lands as **theirs**. The test is not *"was I asked to imitate them"*; it is ***"will a human read this as coming from them"***. Measured, workers asked for a Slack message, a release note and a repo description climbed to `declared/` **once in three** — all three were operator-facing and none announced it. If you are writing prose a person will read, assume yes. Voice is the one thing whose absence you **cannot detect in your own output**: under-read `observed/` and the gap shows as work visibly missing something; under-read `declared/` and it comes back fluent, correct and **not theirs**, with nothing looking wrong. Only one of those announces itself. **One venture's folder** when the task is about that venture — `ventures/` is *partitioned*, so the floor lists them and you open the one that applies; **`about_business.md` does not substitute** (~1,000 words summarising over 100,000; § IV says the detail lives in each venture's own files). Costs: `hooks/context-rungs.py`.
   ...
   **d. Unsure is not a third answer — read more, not less.** Over-reading costs tokens on one session; under-reading costs the operator's voice and fails silently.
   ...
   > **Why this shape, and why no volume is written down here.** Reading everything is six figures of tokens on a large vault before the first question — which is why, measured across spawned workers, the rule was producing anywhere from 0 to 18 files, with no floor underneath and nothing reporting the zeroes. The opposite failure is just as real: a worker told to read *all* of both folders for a two-sentence task spent 38 tool calls and never wrote the two sentences. But the deeper bug is that **"read everything" was correct when it was written** — a fresh clone's entire context is smaller than this page — and it silently stopped being correct as the vault grew, because nothing ever measured. A constant baked into a rule about a quantity that grows works, then doesn't, and nobody is told. So the sizes here are named as ratios and derived on demand, never stated as numbers: a newcomer should genuinely read all of it. And the decision is made by the **shape of the work and the size of the vault, never the identity of the session** — a rule keyed on who is running would make every spawned agent permanently thinner than the primary session, including the ones whose whole job is to sound like the operator.
...
>
> The auto-mode permission classifier reads `spawn <name> …` as *"launch an autonomous agent"* and auto-denies it silently (a red dot, no prompt), and the keystroke path it would fall back to leaks synthetic keystrokes. **Un-sandboxing does not help — the classifier gates it, not the sandbox** (worth stating because a denial is exactly the signal that otherwise tells you to retry with the sandbox off; here that retry is denied again). So `spawn`/`spawn-kill` are for **humans** at a terminal and for a surface's own use. As an agent:
>
> - **Need a worker → write a request file, never run `spawn`.** `Write` `~/.aios/spawn-inbox/<name>.json` = `{"name":"<kebab>","task":"<first prompt>"}`, plus optional `"model":"<id>"` **or** `"tier":"<rung>"` (by cognitive load — the `orchestration-ladder` skill picks the rung, [`MODEL-ROUTING.md`](./MODEL-ROUTING.md) defines it), plus `"surface"`. Whichever surface is running — AIOS Glass in the IDE (`vscode.createTerminal`) or the AIOS App, on every platform either ships to — fulfils it natively: no keystrokes, no gate. Writing a file is not gated. **The verbs are identical on both; there is no surface-specific dialect to learn.**
> - **A surface is available when one is ALIVE, not when the directory exists.** `~/.aios/spawn-inbox/` persists forever once created, so its presence proves nothing: an operator who closed the IDE or quit the App has the directory and no fulfiller, and a request written into it is never picked up — **not** dead-lettered, just unclaimed, which nothing watches for (`/today` and `/close-day` surface `.undelivered`, not unclaimed). Test liveness instead: each surface publishes `~/.aios/surfaces/<surface>.json` carrying its `pid`, so **a surface is available iff some such file names a pid that is still running.** A stale file outlives its process — measured on a live machine, one surface's file sat three weeks dead beside another's live one. Same artifact you walk for `"surface"` derivation below; here you are asking *is anyone home* rather than *which one am I in*.
> - **`spawn` should carry `surface`; `send`/`kill` need not.** A `send`/`kill` names a session that already exists, so a mis-race self-corrects (a surface that can't reach it hands it back, bounded). A `spawn` cannot — nothing exists yet, and it opens wherever the race lands (an IDE request served as an App pane). Derive it: each surface publishes its process-tree root at `~/.aios/surfaces/<surface>.json`; walk your own ancestry and **compare pids, never process names**. No match is a legitimate answer, not an error — omit `surface` and let them race. Addressing a `send` is still the fastest path when you know where the target lives.
> - **Never hand-write a field beginning with `_`.** `_claim`, `_tried`, `_releases` and `_undelivered` are the fulfillers' own bookkeeping (`_tried` is the list of `surface:pid` ids that already attempted a request); a request arriving with one populated is misread.
...
> - **Reopen a session that is CLOSED** — `{"action":"resume","name":"<kebab>","prompt":"<first prompt>"}`. This is the verb that makes § The Belief operational rather than aspirational: a `spawn` gives you a fresh **something**, and `resume` gives you back the same **someone**, still holding the context it earned. Reach for it whenever the work is a continuation — a PR that came back with comments, an overnight thread, anything where starting from zero means re-explaining what that session already knew.
>   - **Its name resolves from the TRANSCRIPTS** (`~/.claude/projects/*/*.jsonl`), not from the session registry — the registry lists only *running* sessions, so it structurally cannot answer for the one case this verb exists for. A session that renamed itself resolves by its **last** `agent-name` record: the same record the Mandatory First Action at the top of this file already greps, read last rather than first.
>   - **No transcript → a dead letter naming the miss.** It never silently spawns something new in place of what you asked to reopen. Opt into that fallback explicitly with `"fallback":"spawn"` — a **string**, not a boolean, so a second fallback later needs no second field.
>   - **Already running → it is revealed and your prompt delivered**, exactly as `send` would. Resuming a live session would give one identity two processes, which is the same duplication reached from the other direction.
> - **Addressing a LIVE session — the session registry is the only truth:** `~/.claude/sessions/*.json` (each: `name` · `pid` · `status` · `sessionId`). **Never `pgrep`, never a terminal tab title** — a resumed session keeps its old title and reads as dead. **The registry is authoritative for `send` and `kill` and deliberately cannot answer for `resume`:** it holds running sessions, and a closed one is absent from it by design, not by omission. That is why `resume` resolves from the transcripts instead — and why "absent from the registry" must never be read as "no such session ever existed". Keep `prompt` on one line. **A prompt over ~1024 bytes is not delivered as text:** the surface writes it to `~/.aios/bus-payloads/<name>-<ts>.md` and types a single pointer line — **if you receive a pointer line, read that file and follow it; never act on the pointer alone.** The ceiling differs per surface and truncation is silent everywhere, so length is a protocol invariant. A request file that disappears was **picked up**, not done — verify from the transcript (`~/.claude/projects/*/<sessionId>.jsonl`). **`<name>.json.undelivered` means the work did NOT happen and nobody was told** — treat a dead letter as a task still owed (`/today` and `/close-day` surface them). The protocol's authority is `~/.aios/spawn-inbox/README.md`, rewritten by whichever surface boots so it cannot drift from the running code; this section is only what you decide *before* opening it. Also the `orchestration-ladder` skill.
> - **⚠️ A verb you WRITE and spell wrong is now refused, not downgraded.** The four known verbs are **`spawn` · `send` · `kill` · `resume`**. An **absent** `action` still means spawn — the original `{"name","task"}` form is untouched and keeps working — and `{"action":"spawn", …}` written out is accepted too. Case and surrounding whitespace do not matter (`"Resume"` and `" resume "` both resolve), so a refusal means a genuinely unknown verb, never a formatting slip. But an `action` that is present and is not one of the four known verbs is **dead-lettered naming what was written**, instead of quietly falling back to spawn. This is a real behaviour change and it is in your favour: before `resume` existed, degrading an unknown verb to spawn was harmless, because spawn was the only thing it could have meant. Now the two outcomes differ — a fresh *something* versus the same *someone* — so a typo'd `"resmue"` would have silently handed you a brand-new session in place of the one you asked to reopen, with its context gone and nothing reporting the substitution.
> - **Check the contract stamp before assuming a verb exists** — and grep for what is actually in the file. The fulfiller stamps `~/.aios/spawn-inbox/README.md` with an HTML comment reading **`aios-spawn-inbox: contract N`**; the verb set above is contract **3**. A fulfiller still on **2** that receives a `resume` spawns a duplicate rather than refusing it, so the number tells you which rules the surface on *this* machine follows. (Do not look for a literal `INBOX_CONTRACT` — that is the symbol's name inside the App and Glass source, not text that appears in the README.) `"surface"` is how you insist on a particular fulfiller instead of letting the request be raced for.
> - **Two surfaces, two independent implementations of one protocol — there is no Glass inside the App.** A fix ported into one and not the other, or a verb one side removed, is the expected shape of the bug, not a surprise.
> - **No live surface** (no `~/.aios/surfaces/*.json` names a running pid — the normal case for a plain clone with no surface installed, and the temporary case for anyone who closed theirs) → do **not** call `spawn`. Hand it to the operator — a surface's **"Spawn a session"** control, or `spawn <name> "<task>"` in a fresh terminal — with the exact name + task so it's one paste. **The tell you got it wrong:** a red-dot *"tool use was rejected"*, or a `spawn` that opens no terminal.
