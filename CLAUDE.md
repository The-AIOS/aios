...
4. After greeting, load context **on demand, not in full** — and *do* it, as the first tool calls of the session, rather than describing it. List `vault/00 - notes/context/declared/` and `…/observed/`, and read their `_index.md` (a one-line map each). Then read the `## ` headings of `observed/growth.md` and `observed/patterns.md` (`grep '^## '` — tens of lines, not hundreds): **those two are a floor, read every time**, because they hold what the operator tends to avoid or repeat and *no task ever names them*. Then read **only the files this task touches** — before acting, not after. When the task is vague, grep the context folder for the literal subject first, then read what matches. `INTENT.md` (repo root) when the task acts on the operator's behalf. *Why not the full ritual:* measured on a live vault, three of three tasks reached the same or better answers on demand as with every file preloaded, at roughly a third of the cost — a preload writes every file to the cache on the first request. *Why the floor is not optional:* "read what the task touches" hands a fresh worker the relevance call, which is the one judgment it is worst placed to make — it cannot know that a file the task never mentions is the one that makes an answer *this operator's* rather than merely correct. The interactive **primary** session keeps the full ritual: the case for preloading is cross-file synthesis that no question triggers, and that case is untested.
