...
4. After greeting, **load context — and *do* it, as the first tool calls of the session, rather than describing it.**
   ...
   **It reads the tail because the two folders are written differently.** `declared/` is **restated** — identity, rewritten in place, no chronology, so there is no newest end to read. `observed/` **accumulates** — dated and append-ordered, so its newest end is exactly what the last sessions wrote. The tail is also **bounded**: at ten times the entries it reads the same five per file while the headings still index all of them, and **a bounded selector never goes stale where an unbounded volume does**. Recency is not relevance — an older entry that matters is one title away, and `antifragile.md`'s titles are all in front of you, which is what § Session Start's *scan before executing commands* needs.
