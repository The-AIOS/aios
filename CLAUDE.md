...
1. **Snapshot before editing, stamp after (mandatory — one rule, both halves).** Before modifying any observed-context file, archive the prior version with **`hooks/aios-snapshot`** (feeds `/aios:trace` + `/aios:drift`):
   ...
   It prints one line per file — `snapshot <dest>` (archived) · `identical <dest>` (an existing archive already holds this exact content; nothing written, and that is success) · `collision <dest>` (differed from every existing variant, took the next free letter). **Do not hand-roll the `cp`.** The destination is keyed by *day and filename* with **no writer in it**, so a second session archiving the same file the same day lands on the first one's destination — and `cp` exits 0, making a colliding write byte-for-byte indistinguishable from a successful one. It surfaces later as a hole in the one artifact whose entire job is to survive.

   The compare-then-take-the-next-letter protocol this replaces was correct, and it held every time it was followed — but it was a **human-executed mutex**, and two of its failure modes no amount of care removes: it is **not atomic** (two sessions can each observe the same free letter and both take it), and it compared only against the **base** name (identical content already archived under `b` still minted a `c`). The tool takes both decisions under a real lock and compares against *every* existing variant. Measured with a realistic decide-to-write window: six concurrent writers keep **1 of 6** contents without the lock and **6 of 6** with it (`tests/aios-snapshot.test.sh`, whose control fails loudly if the race ever stops being reproducible).

   Not hypothetical, and the rate is what settles it: **2026-08-14 alone produced four suffixed archives** across three session blocks and two sessions, with the day's last close finding a `b` pair already sitting on its destination. A single day has run to **eight** session blocks. The one-session-per-day assumption the path encodes stopped being true long ago. *(Deciding whether a file is too stubby to be worth archiving stays the caller's call — the tool archives what it is told, rather than hiding a skipped archive behind a success.)* **Then, in the same edit, set that file's `updated:` frontmatter to today's date.** The two halves are inseparable: the snapshot preserves the *old* version, the stamp dates the *new* one. This is load-bearing — the streak-independent staleness alarm (`/today` + `/close-day`) reads *only* `updated:`, so a write that edits the body without bumping the stamp feeds the one reliable backstop a lie (it cries wolf on a fresh file, and could stay silent on a genuinely stale one). The rule, mechanical: *if you touched an observed file's content, its `updated:` reads today — no exceptions.* (When the tool is Obsidian MCP, `update_frontmatter` sets `updated:` without disturbing the body — a `patch_note` on the body alone is exactly how the stamp silently drifted.)
...
| `vault-routine.md` | Cadence actually changed — a ritual stopped landing, or a command's rhythm was re-aimed | **Restated, not appended**: rewrite the affected line in place. This file is a spec, not a log — it carries `restated: true` and is exempt from the staleness clock ↓ |
...
**The closing rule — an entry names the mechanism that now catches its class, or says why none can.** `antifragile.md` is re-read at every session start forever, so an entry that records a lesson without changing anything is a permanent cost with no payoff: it can only fire on the runs where someone reads and remembers it. So **an entry may not close until it names the mechanism that now catches its class — a check, a hook, a template or structural change that makes the failure impossible — or an explicit reason no mechanism can.** *"No mechanism can"* is a common and fully legitimate answer: judgment calls, one-offs, resulting-fallacy and operator-behaviour lessons genuinely cannot be caught mechanically. They say so in one clause and move on. What the rule forbids is **silence** — with it you cannot tell an entry whose class is already caught from one whose fix nobody wrote.

**The mechanism is whatever your setup can enforce.** Framework contributors usually name a `tests/` gate; an operator's vault has no CI, so theirs is usually a hook, a command's own check, or a structural change that removes the possibility. The rule is about *enforcement existing*, never about a particular tool — and it is not a licence to invent gates: the ones worth having come from incidents, which is the right generator. This makes that generator mandatory rather than incidental.

**The payoff is compaction that loses nothing.** Today, condensing an entry trades away real wisdom, because the wisdom lives only in that prose. Once a mechanism holds the class, the prose is redundant: the entry can shrink to its number plus a pointer, **because the mechanism is now the memory.** That is the lever `/aios:compact` Step 3.5 otherwise lacks.

**System mindset:** don't just name the failure — diagnose the system, not the symptom. Ask *"Was the decision process flawed, or good decision-making that produced a bad outcome?"* (avoid resulting fallacy). Format: what happened, why it broke, system fix, category. Log systemic mistakes, not every small one. Never delete entries; supersede. Evolution is the value. **Bounded:** read every session, so it carries a size cap — but **tokens are the real cost; entry count is only a proxy for it**. **The numeric bound lives in `/aios:compact` Step 3.5 and nowhere else — read it there, and do not restate it here.** (That is not style: this line used to carry the numbers, and on 2026-08-18 an operator's report led to removing a *second* copy from `housekeeping.md` — after which this third copy was still sitting in the file every session loads at startup, which is the most authoritative-looking place a stale bound can hide. One bound, one home.) `/aios:compact` (Step 3.5) snapshots, then works two levers: **tombstone** entries the file itself marks graduated/superseded (keep the number + title + pointer so cross-references still resolve), and **condense** verbose entries into what-broke / why / fix triplets — preserving every entry number, the meta-pattern index, and all exact commands, paths and hashes. *Condensation is the primary lever:* once entries are already terse, removal frees almost nothing and costs real wisdom. *"Never delete"* → **"never delete without a snapshot."** History lives in the snapshot, never in machine-local memory.
...
**A third shape: restated files have no clock.** Most `observed/` files **accumulate**, which is what makes a staleness threshold meaningful. A **restated** file does not — it is a specification rewritten in place, and its being unchanged is the system working, not rotting. Left on an accumulation clock it raises an alarm precisely when the thing it describes is stable, every month, forever — which is how operators are taught to ignore alarms. Such a file declares `restated: true` in its frontmatter and the alarm skips it (precedent: `exempt-line-check: true` on project notes). **Declare it explicitly rather than sniffing for undated headings**: the context floor may infer a file's shape because it only chooses what to *display*, whereas this decides whether to *suppress a warning*, and a suppression that fires on a heuristic is one nobody can audit. The flag is only half — a restated file still needs a **write trigger** in the table above, or exempting it is just muting an alarm.
