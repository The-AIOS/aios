...
4. After greeting, **load context — and *do* it, as the first tool calls of the session, rather than describing it.** How much you load depends on the work, not on the fact that you are a worker.

   **a. The floor, always, whatever the task — and it is cheap.** List `vault/00 - notes/context/declared/` and `…/observed/` so you know what exists. Then read the **`###` entry titles** of `observed/antifragile.md`, `observed/preferences.md`, `observed/patterns.md` and `observed/growth.md` (`grep '^### '`). Titles only, not bodies — a few hundred lines, roughly 4% of what those four files contain. This is unconditional because those four hold what the operator tends to repeat, avoid, prefer, and has already been burned by, and **no task ever names them.** `antifragile.md` especially: § Session Start calls it the file to *scan for relevant rules before executing commands*, and an entry titled like `110. Empty and no-op inputs resolve to something REAL` tells you both that the lesson exists and exactly when to open it. **Read `##` headings instead and you get ten section names and nothing usable** — the index that matters is one level down.

   **b. Know what you did NOT read, and that you may go back for it.** The full declared + observed set is roughly **150k tokens** of accumulated context about this operator — voice, ventures, psychometrics, relationships, every correction the system has absorbed. You are not loading all of it now because most tasks do not need it, **not because it is off limits.** The folder listing from (a) is your menu. **Opening any of it the moment the work turns out to need it is expected, not exceptional** — if you find yourself guessing at how they would put something, or who someone is, or whether they have decided this before, stop guessing and go read. The cost of one extra file mid-task is trivial; the cost of inventing what was already written down is that the work stops being theirs.

   **c. Then one question about your own output — not about the files.** *Will what I produce be read as the operator's own words, or act on their behalf?*

   - **Yes → run the full Session Start Ritual** (§ Rituals → Session Start), every declared and observed file. Writing, deciding, advising, representing, anything an audience will attribute to them, anything touching a venture or a relationship.
   - **No → the floor plus only what this task touches.** Work whose correctness is checkable without knowing the operator: code, tests, file operations, data transforms, mechanical sweeps. When the task is vague, grep the context folder for its literal subject and read what matches.

   **d. Unsure is not a third answer — run the full ritual.** The two errors are not symmetric. Over-reading costs tokens on one session. Under-reading costs the operator's voice, and it fails *silently*: the work comes back fluent, correct, and not theirs, with nothing to flag it.

   `INTENT.md` (repo root) whenever the task acts on the operator's behalf — that is the trust contract, and (c)-yes work always needs it.

   > **Why this is the shape.** The full ritual is ~150k tokens before the first question, which is why, measured across spawned workers on a live vault, **only 4 in 28 ever came close to it and 3 read nothing at all.** "Read everything" was not producing everything; it was producing an unmeasured fraction with no floor underneath. So the floor is made cheap enough that it always happens, what is skipped is named so it can be reached for, and the full read is routed to by the **shape of the work, not the identity of the session** — the same axis the tier ladder uses (§ Spawning Sessions → Model tier). A rule keyed on who is running would make every spawned agent permanently thinner than the primary session, including the ones whose whole job is to sound like the operator.
