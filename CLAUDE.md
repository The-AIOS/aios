...
**A headless tool allowlist needs `--permission-mode` beside it, or it is advisory.** When you run `claude -p` for anything that must not act, `--allowedTools` alone does **not** hold: measured with `permissions.defaultMode: "auto"` in `settings.json`, a run passing an allowlist plus `--strict-mcp-config` still performed `Bash` and `Write` calls with `permission_denials: []`. Adding `--permission-mode default` (or `plan`) blocks it. Prompt FIRST, then the flags: `--allowedTools` is variadic and swallows a prompt that follows it. And **never verify this by asking the agent what tools it has** — under a restrictive allowlist it still lists `Bash` and `Write`, because it is describing its *schema*, not its permissions. **Only an absent side effect is evidence.** Full reasoning: [`MODEL-ROUTING.md`](./MODEL-ROUTING.md) § Verify an id before you trust it.
