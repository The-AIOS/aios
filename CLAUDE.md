...
**Structure evolution follows the vault grammar.** The Router places *new* files; how existing structure *evolves* — retiring notes (`_archived/`, record vs lifecycle species), naming folders, what a rename owes the graph (`aliases:`, redirect stubs) — is specified in `templates/aios/vault-grammar.md`. Consult it before archiving, renaming, or restructuring; execute anything beyond a handful of files via the `vault-migrations` skill (dry-run → inventory-verify → write). `/aios:housekeeping` (Bucket 21) audits against it.
