...
4. After greeting, **load context — and *do* it, as the first tool calls of the session, rather than describing it.** How much you load depends on the work, not on the fact that you are a worker.
   ...
   **a. The floor, always, whatever the task.** List `vault/00 - notes/context/declared/` and `…/observed/`, read their `_index.md` (a one-line map each), and read the `## ` headings of `observed/growth.md` and `observed/patterns.md` (`grep '^## '` — tens of lines, not hundreds). Those two are unconditional because they hold what the operator tends to avoid or repeat and **no task ever names them**. A worker that reads nothing is the failure this floor exists to end.
   ...
   **b. Then one question about your own output — not about the files.** *Will what I produce be read as the operator's own words, or act on their behalf?*
   ...
   - **Yes → run the full Session Start Ritual** (§ Rituals → Session Start), every declared and observed file. Writing, deciding, advising, representing, anything an audience will attribute to them, anything touching a venture or a relationship. **Most bundled agents are in this class** — a `content-writer` that skipped `personal_voice.md` because the task did not name it produces competent prose that is not theirs, and nothing reports it.
   ...
   **c. Unsure is not a third answer — run the full ritual.** The two errors are not symmetric. Over-reading costs tokens on one session. Under-reading costs the operator's voice, and it fails *silently*: the work comes back fluent, correct, and not theirs, with nothing to flag it.

   `INTENT.md` (repo root) whenever the task acts on the operator's behalf — that is the trust contract, and (b)-yes work always needs it.

   > **Why this is the shape.** Measured on a live vault, several tasks reached the same or better answers loading on demand at roughly a third of the cost — a preload writes every context file to the cache on the first request. That saving is real and worth taking **where the work does not need the operator in it**. What it cannot support is a rule keyed on *who is running*: that would make every spawned agent permanently thinner than the primary session, including the ones whose entire job is to sound like the operator. So the axis is the **shape of the work, not the identity of the session** — the same axis the tier ladder already uses (§ Spawning Sessions → Model tier: *pick a rung by the shape of the work, not its importance*).
