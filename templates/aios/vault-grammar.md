...
- **The loop comes wired — the layer surfaces daily the moment it exists.** `/aios:close-day` detects live roadmap files, sweeps freshly-flipped ✅/❌ rows into each map's `## ✅ Done & dropped` ledger (foldable; maps show what's ahead), and regenerates a `## Roadmap board` in `projects/_index.md` from the top open rows; `/aios:today` renders `## The Board` in the daily note from it. A clean roadmap nothing surfaces is worse than no roadmap — recommending the layer means recommending the loop, and the framework carries both.
