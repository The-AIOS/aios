...
function spawn {
    ...
    $taskPreamble = (@(
      ...
      'INCLUDES THE ONE BELOW IT -- climbing adds to the floor, never replaces it. Both',
      'bounds are reachable:',
      '',
      '  MIN -> the map is enough. Code, tests, file operations, data, mechanical sweeps --',
      '         work whose correctness is checkable without knowing the operator. Open an',
      '         observed/ entry the moment one of its titles tells you it applies.',
      '  MAX -> every file in declared/, observed/ AND every venture, read whole. Not a',
      '         failure state. Right when',
      '         either (i) the whole context is cheap -- run context-rungs.py and if its',
      '         verdict says read all of it, read all of it; or (ii) THE TASK IS THE',
      '         CONTEXT ITSELF: synthesising across their history, auditing or compacting',
      '         the observed files, deriving a pattern that only shows across many of them,',
      '         or answering AS them. An index cannot serve those; the corpus is the input.',
      '',
      ...
      '         SMALL folder, and voice is the one thing whose absence you cannot detect in',
      '         your own output. Under-read observed/ and the gap shows as work visibly',
      '         missing something. Under-read declared/ and the work comes back fluent,',
      '         correct, and NOT THEIRS, with nothing in it looking wrong. Only one of the',
      '         two failures announces itself. Do NOT also preload all of observed/ for this',
      '         -- the map above is the way in, and a two-sentence task does not need the',
      ...
      'it will not apply to this task, NOT because it is off limits. Open any of it the moment a',
      'title or the work points at it: a task touching',
