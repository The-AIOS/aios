...
function spawn {
    ...
    # The task file opens with the context preamble -- the same rule the bash wrapper prints,
    # because a Windows worker is governed by the same contract.
    #
    # A STRING ARRAY, not a here-string: this function lives inside the single-quoted
    # here-string $WRAPPER, and PowerShell offers no way to escape a closing terminator
    # inside one -- a nested single-quoted here-string ends the OUTER one at its first
    ...
    $taskPreamble = (@(
      ...
      '',
      'It emits both _index.md, EVERY HEADING in both folders, THE NEWEST 5 three-hash',
      'ENTRIES OF EVERY observed/ FILE IN FULL -- newest by the DATE each entry carries,',
      'not by file position, since the tail is the newest in only about 5 of 8 files.',
      'One exception: a RULE LIBRARY like antifragile.md emits its meta-pattern INDEX',
      'instead, because an entry from four months ago binds as hard as one from this',
      'week and its job is to fire BEFORE the mistake. Plus INTENT.md -- which is at the floor',
      'because it governs what you may DO, a different question from whether your output',
      'sounds like them, and the one that binds a mechanical worker hardest.',
      'Absent the hook, glob it by hand --',
      'grep -h ''^#'' on declared/*.md, grep -h ''^###'' on observed/*.md, then the tail of',
      'each observed file -- but prefer the hook: a floor assembled by hand gets done',
      'partially or skipped, and afterwards nothing can tell the difference.',
      '',
      'The floor carries LEARNED CONTENT, not only titles. Headings tell you what exists;',
      'they tell you nothing the system has learned. Every close-session and close-day',
      'appends to observed/ -- if the next session never reads what was appended, the loop',
      'does not close. It reads the TAIL because the folders are written differently:',
      'declared/ is RESTATED (rewritten in place, no newest end), observed/ ACCUMULATES',
      '(dated and append-ordered). The tail is BOUNDED -- at ten times the entries it reads',
      'the same five per file while the headings still index all of them. Recency is not',
      'relevance: an older entry that matters is one title away, and antifragile titles are',
      'all in front of you, which is what scan-before-executing-commands needs.',
      '',
      'BOTH FOLDERS VARY FROM VAULT TO VAULT: the framework ships five declared files and',
      'nine observed ones and operators rename them, add their own, and write them in their',
      'own language, so a named list silently skips whatever it does not mention. Measured,',
      'a floor naming four observed files found ZERO titles on a renamed vault -- and a',
      'floor at zero is indistinguishable from one that fired.',
      '',
      'ABOVE THE FLOOR THERE ARE NO TIERS -- ONLY FIT. Intelligence is the right',
      'information at the right time, not the largest pile you can afford. Nothing to',
      'qualify for, no level to reach: open what the work points at, to the depth it',
      'needs, when it points there.',
      '',
      '  DEPTH IS YOURS TO SET, per file, without asking. The floor gave you five recent',
      '  entries per observed file; if a title above them is what the task turns on, open',
      '  that entry, or twenty, or the file. If ONE declared file answers your question,',
      '  read that one rather than the folder. The floor is a starting position, NOT an',
      '  allowance. If you catch yourself thinking "I am only at the floor, so I should',
      '  not read that" -- that is the misreading. There is no permission gate above it.',
      '',
      '  LATER BEATS SPECULATIVELY EARLIER. Reading declared/ at minute thirty, having',
      '  just discovered the deliverable goes out in their name, is better fit than',
      '  reading it at minute one in case it might. Reaching mid-task is the NORMAL MODE,',
      '  not a fallback. What you cannot discover at the moment of need is what you never',
      '  knew existed -- the floor exists for exactly that.',
      '',
      '  TWO DEPTHS WORTH NAMING, as costs rather than permissions:',
      '    ALL of context/declared/ -- when what you produce will be read as the words of',
      '      the operator, or act on their behalf. OPERATOR-FACING IS FAR BROADER THAN',
      '      GHOSTWRITING, and this is where the question gets misread. If a human will',
      '      read your prose and it carries their name or their team name -- a message, a',
      '      post, a note, a release note, a README line, a repo description, an email, a',
      '      reply, a doc -- it lands as THEIRS. The test is NOT "was I asked to imitate',
      '      them"; it is "WILL A HUMAN READ THIS AS COMING FROM THEM". Measured: workers',
      '      asked for a Slack message, a release note and a repo description climbed to',
      '      declared/ once in three. All three were operator-facing; none said so.',
      '      Writing prose a person will read? Assume yes.',
      '      the operator, or act on their behalf. It is the SMALL one, and voice is the',
      '      one thing whose absence you cannot detect in your own output: under-read',
      '      observed/ and the gap shows as work visibly missing something; under-read',
      '      declared/ and it comes back fluent, correct, and not theirs, with nothing in',
      '      it looking wrong. Only one of the two failures announces itself. (Do NOT',
      '      also preload all of observed/ for this.)',
      '    ONE VENTURE folder -- when the task is about that venture.',
      '',
      '  READING EVERY FILE IN declared/, observed/ AND EVERY VENTURE IS NOT THE TOP OF A',
      '  LADDER. It is the degenerate case, correct in two situations only: the whole',
      '  context is cheaper than deciding what to skip (run context-rungs.py; if the',
      '  verdict says read it all, read it all), or THE TASK IS THE CONTEXT ITSELF --',
      '  synthesis across their history, auditing or compacting the observed files,',
      '  answering AS them. Outside those two, reaching for everything is not',
      '  thoroughness, it is the absence of a judgment.',
      '',
      '  The map is enough for work checkable without knowing the operator: code, tests,',
      '  file operations, data, mechanical sweeps -- until it is not.',
      '',
      'DOES THE TASK TOUCH A VENTURE? Read that venture folder from context/ventures/.',
      ...
      'venture own files. Reading the summary and believing you have venture context is',
      'mistaking the index for the territory.',
      '',
      'EITHER WAY THE REST STAYS OPEN TO YOU -- that is what the map is for. You hold the',
      'entry TITLES of observed/ and its newest entries, not its older bodies. That is an',
      'index-then-open pattern, NOT because any of it is off limits. Open any of it the',
      'moment a title or the work points at it: a task touching a person or a venture',
      'usually wants ecosystem.md or business.md; unsure how they like something done,',
      'preferences.md. Reaching mid-task is expected, not exceptional. If you catch',
      'yourself guessing at how they would phrase something, who someone is, or whether',
      ...
      'costs tokens once; under-reading costs the voice of the operator and fails silently.',
      'If the call is genuinely hard, load the right-context skill -- it carries the full',
      'ladder and measures what each rung costs in THIS vault (hooks/context-rungs.py).',
      '',
      'Describing this rule is not doing it: right after the CLAUDE.md identity check, the next',
