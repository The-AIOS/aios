...
function spawn {
    ...
    $taskPreamble = (@(
      ...
      'both folders, read both _index.md, then read EVERY HEADING IN BOTH FOLDERS -- glob them,',
      ...
      'it. antifragile most of all -- it is the file to scan for relevant rules before',
      'executing commands. BOTH FOLDERS VARY FROM VAULT TO VAULT: the framework ships five',
      'declared files and nine observed ones and operators add their own, so a named list',
      'silently skips whatever it does not mention. The glob cannot go stale.',
      '',
