...
**Then the worker judges the read against the task — a range, not a switch.** The map is the minimum. **Every file in both folders, read whole, is the maximum, and it stays reachable**: right whenever the whole context is cheap — on a lightly-written vault it costs less than a short document, and the worker is told to just read all of it — and right at *any* size when **the task is the context itself**, such as synthesising across your history, auditing or compacting the observed files, or answering *as* you. An index cannot serve those; the corpus is the input. The question that settles most of what lies between: *will this be read as your words, or act on your behalf?* **Yes → all of `declared/` plus `INTENT.md`. No → the map is enough.** **Size decides, never the vault's age** — a long-standing vault whose owner writes little stays small, and a three-month-old heavy one does not. The reason that split holds is an asymmetry in how the two failures surface: under-read `observed/` and the gap shows up as work visibly missing something, or a question the worker knows to ask. Under-read `declared/` and the work comes back fluent, correct, and **not yours** — with nothing in it looking wrong. **You cannot detect the absence of a voice from inside your own output.** Only one of the two failures announces itself, and that is the one worth paying to avoid.
