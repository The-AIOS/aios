...
**Why this survived a CI guard that exists for it.** The validation suite has checked for references to the retired vault path since the extraction — but it grepped for the path as **text** (`~/obsidian`, `$HOME/obsidian`, `/Users/*/obsidian`). The real hardcode was *assembled programmatically* from the home directory, so not one pattern matched: the guard reported "no hardcoded paths" about a state it had never examined, and stayed green for two months over three live occurrences. A second check now covers the constructed form in `.py`/`.sh`/`.ps1` (and was verified against the buggy file before shipping — a guard that passes on the bug it exists for is worse than no guard, because it also tells you you're safe). The lesson generalises past this path: **a check that knows only one spelling of the thing it forbids is not a check.**
