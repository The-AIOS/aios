...
*Two more surfaced on the first CI run, and both are the same lesson pointed at the author. The `.ps1` carried **553 non-ASCII bytes with no BOM** — which PS 5.1 corrupts at parse time, and which the existing guard caught immediately; the local check that had cleared it was `grep`, which reports **zero matches** on a file it decides is binary, so the pre-flight was a false negative and the only honest instrument was a byte count. And the new suite's own control was **environment-dependent**: `unknown` has five causes, so on a runner with `gh` installed but unauthenticated the missing-tracker assertion passed for entirely the wrong reason, and its control correctly reported itself vacuous. Both now assert on the `reason` field, which names **which** guard fired — the only part of the output that distinguishes them. Verified by re-running the suite against a deliberately unauthenticated `gh`: 31/31 in both environments.*
