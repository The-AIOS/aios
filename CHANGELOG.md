...
>
> **Never leave an operator to judge code alone — always recommend.** A bare `[y / n / show me the whole file]` asks a non-technical person a question they cannot answer (*"how would I even tell?"*), and an unanswerable prompt gets answered by closing the window. So **recommend yes** in the ordinary case, and do not hedge it — a recommendation followed by *"but nothing breaks if you skip it"* is an invitation to skip, and it is also frequently untrue, because the change may be the fix.
>
> **What you may base that recommendation on, and what you may not.** Recommend on **what declining costs**: the installation drifts, and later fixes arrive on a base that no longer matches. That is a fact about the framework and it is yours to state. Never recommend on **what the diff contains** — *"this looks fine"* is a verdict on code you skimmed, and it is the single sentence that would make every future disclosure worth less than this one. If you catch yourself wanting to say the change looks harmless: **say what it does instead**, and let the recommendation rest on drift.
>
...
**Nine fixes contributed by an operator, each checked and extended so vaults shaped differently are covered too** ([#175](https://github.com/The-AIOS/aios/pull/175)–[#183](https://github.com/The-AIOS/aios/pull/183)). The pattern they share is a check that could fail without saying so:
- `/aios:update` treated a hash it could not compute as a result, could reorder `.gitignore` rules, and read a missing folder as "no drift". Each now fails closed, and it hashes with `sha256sum` where `shasum` is missing. Two updates at once no longer delete each other's clone; a crashed one no longer blocks the next.
...
- `/close-day` redraws `ecosystem.md` on its own clock, so a map that gets a small note every few weeks is still redrawn; it reads insights Tier A already routed, finds reports by the date it is closing, and snapshots through the helper.
- `/aios:housekeeping` stops reporting every vendored source as behind.
...
> **What you can now do.** Ask the `security-engineer` agent to review a codebase with Cloudflare's security-audit method. Run a headless routine that uses Bash under the sandbox without it approving every command or reading your credential folders. Trust the dates in your morning plan, which now checks the next 14 days of your calendar. Nothing changes on your machine by itself: the sandbox settings are opt-in, and they go in a file only your routines load.

**The `security-engineer` agent can now run a real source-code audit.** AIOS now bundles Cloudflare's `security-audit` skill (MIT), unmodified, in `skills/cloudflare/`: reconnaissance, playbooks per attack class, an evidence bar every finding must clear, and a coverage record of what was *not* looked at. The agent uses it for guidance by default, reading only the playbook a question needs. It runs a full audit only when you explicitly ask for an audit or a pen-test, and it proposes how deep to go, and what that costs, before it starts. Audit output stays outside your vault and outside the repo being audited; only a summary goes into the project note. The skill registers itself on update. That needed a fix of its own: updates registered new skills only from the three folders that already existed, so a newly bundled one would have synced and never loaded.

**A routine that runs Bash under the sandbox had less containment than it looked.** Two findings, reported privately by an operator and reproduced with planted test files. First, the sandbox approves every Bash command by default, so a routine's `--allowedTools` list stopped nothing. Second, sandboxed Bash can read folders outside the project, including where MCP tokens live, and a `Read(...)` deny rule does not stop it. `MODEL-ROUTING.md` now gives the three settings that close both, as a JSON block to copy, and they go in a settings file passed only to the routine (`claude -p … --settings <file>`). **Keep them out of your interactive `~/.claude/settings.json`.** There, auto-approve is what keeps a sandboxed session from asking about every command, and denying a token folder breaks tools that read it, `/today` included. `/aios:housekeeping` now reports headless Bash calls that pass no such file, and never reports your interactive settings. One more thing measured along the way: a Bash routine still inherits your *user-level* allow rules, so keep its `--allowedTools` to what it actually runs.

**Your morning plan now checks two weeks of calendar, not one day.** `/today` used to see only today's events and took every later date from your notes, so a meeting that moved kept its old date everywhere. It now reads the next 14 days every morning. When the calendar has the event on a different date, the calendar wins, the note is corrected and the plan says what moved. When a note names a meeting the calendar doesn't have at all, the plan asks you instead of changing anything. If you have no Google calendar connected, the step is skipped silently. Contributed by an operator ([#170](https://github.com/The-AIOS/aios/pull/170)).

**Two morning alarms stop miscounting.** The observed-context staleness alarm is now a hook. It exempts files frozen by design (give a closed record `status: historical-snapshot`), lists every exempt file so nothing is muted out of sight, and on a fresh install treats a file with an empty `updated:` as not started yet, rather than raising an alarm. The observation-buffer check now counts sections that mix `###` entries and bullet entries. Contributed by the same operator, and adjusted so vaults laid out differently count correctly too.

**Moving an insight out of the buffer could delete the entries below it.** In a section holding both entry styles, routing a `###` entry removed the bullet entries that followed it. Nothing reported it. It was live before this update and is fixed, with a test that fails on the old code.

**Hooks print on Windows again.** A Windows console couldn't print the `→`, `—` and accents that fill a vault, so the context floor crashed on the first one and printed nothing. It looked the same as a floor that had run and found nothing. It is fixed for the floor ([#172](https://github.com/The-AIOS/aios/pull/172), contributed by an operator) and for five other hooks with the same problem, and a check now catches the whole class.

**Smaller fixes.** The account watcher's background check found no Python on macOS on days the system `python3` wasn't working, even with Homebrew's installed. It now looks in the usual install folders too, with no reinstall needed. `voice-gate` refuses to give a density score until you have 5,000 published words for that kind of writing, and says so, while still reporting the tells that need no baseline. Before, a fresh install could get a number measured against nothing. And from a review of AIOS's prompts against Opus 5.5 ([#174](https://github.com/The-AIOS/aios/pull/174), contributed by an operator): the prompt-engineering examples now use `claude-sonnet-5`, the orchestration skill's description is shorter, which trims what every request carries, and the growth companion states its six rules plainly instead of in capitals.

**Action required:** none for most operators. Start a new Claude session after updating so the new skill and commands load. **If you run headless `claude -p` routines that use Bash under the sandbox**, create a settings file with the three settings from `MODEL-ROUTING.md` § Verify an id before you trust it, and pass it to those routines with `--settings`. Leave your interactive settings as they are. `/aios:housekeeping` lists the calls that need it. **If you have your own routines that parse the pre-loaded calendar output**, its header now reads `## Google Calendar — Next 14 days (AUTHORITATIVE — reconcile vault dates against this)`, where it used to say `Next 7 days`.
...
**First-time setup asks for your hands once, and works on Windows without guesswork.** A real Windows install through the AIOS App stopped the operator twice, at two unexpected moments, cloned into a folder it then could not read, and wired its hooks by trial because the instructions named `pwsh` and `python3`, which a stock Windows machine does not run. Setup now asks up front, in one message, for the two things only you can do (a GitHub sign-in and access to your Claude settings folder), clones into `~/aios` rather than beside it, and uses commands that exist on the machine in front of it — the PowerShell you actually use, and whichever Python actually runs. **GitHub is now optional:** without it, setup carries on with your vault on this computer only, and each morning's plan says so in one line until a private backup exists. If your vault already has a private remote — whatever it is named — you will see nothing. Contributed by an operator ([#167](https://github.com/The-AIOS/aios/pull/167)), extended so that nobody whose setup already worked is left out.
...
**If you updated earlier today, one clean-up is owed — and the update does it for you.** That run asked whether to delete five files the Claude API skill no longer has, with *keep* as the default. Keeping them left each language's old single-file guide beside the new folder that replaced it, so a session could read the outdated one. Updates now remove a file canonical deleted when you never edited it, and ask once — listing them together — about any you did edit. **For the five already kept:** for each of `skills/anthropic/claude-api/{csharp,go,java,php,ruby}/claude-api.md` that still exists, delete it **only if** it matches canonical's last copy (`git -C /tmp/aios-update-check show 628ca91:<that path>`); keep any you changed. If none exist, there is nothing to do.
...
>
> **Every fix in this release was a silent one** — no error, no warning, and in three cases output that looked entirely reasonable while being wrong. Five of these came from outside the project.

**On macOS, `/aios:update` could report "current" without comparing anything.** Since 09-15, two of its steps used a shell variable named `path` — and in zsh, the shell every macOS session runs, `path` *is* your `PATH`. The step kept running, but `git` and `diff` were no longer found, and their empty output read exactly like "nothing changed" and "0 drift". No error reached you, and the tracker could advance past files it never applied. The variable is renamed, both steps now **stop** if a tool they need cannot be found instead of reporting a clean result, and a new check runs these steps under zsh on every change. **You do not need to do anything:** the update's final check compares your vault against the framework directly, whatever the tracker says, so your next `/aios:update` applies anything an earlier run skipped. Reported by an operator session. ([#166](https://github.com/The-AIOS/aios/pull/166))

**Your calendar times were right; the plan's reading of them was not.** Google reports an event you own with a local offset, and one you *subscribed* to — which is what an Outlook / Microsoft 365 mirror is — normalised to UTC. The plan read the second as though it were already local, so every mirrored meeting moved by your own offset while the Google-owned ones stayed correct **on the same page**. That is what made it expensive: the plan looked internally consistent, and nothing in it said otherwise. Measured on a live vault, a full working week rendered two hours early. Ordering had the same flaw, so an event just after midnight filed under the previous day. Times now come from the instant itself, converted to the timezone already in `USER.md`. ([#149](https://github.com/The-AIOS/aios/pull/149))

**On Windows, `/aios:update` was reporting nearly every framework file as drift.** Its completeness reconcile compared file contents without normalising line endings, so with `core.autocrlf` checking your vault out as CRLF against an LF clone, everything read as changed — **267 drift lines on a live Windows vault, 266 of them false.** Step 6.5 tells a session to *apply* each such line, so the instruction was to overwrite your own tree. Related, and the reason it went unseen: `command -v python3` succeeds on Windows against a Microsoft Store stub that exits without doing anything, so the Google Workspace connector passed its own precondition and then read empty input from every step. Both are fixed, and the line-ending flag is now probed rather than assumed. ([#146](https://github.com/The-AIOS/aios/pull/146))

**The secret scan skipped whole folders.** `aios-commit` stages a directory's entire subtree, but the scanner only examined arguments that were plain files — so a token inside a folder you passed by name went into the commit unscanned, while the same file passed directly was blocked. It now expands a directory to exactly what a commit of it would carry, reads symlinks as the link text git actually stores, and knows the `gho_` token `gh auth login` mints. Separately, `pre-push` let a trailing slash in a remote URL bypass the blocked-owner check, and printed the URL — credentials included — into its refusal message. Both closed. ([#157](https://github.com/The-AIOS/aios/pull/157))

**If you rotate between several Anthropic accounts, rotation could lose a login.** Two switches overlapping — two watcher ticks, or a manual switch beside an automatic one — both read the outgoing account, and the second saved the incoming credential under the outgoing account's name. That account's saved login was then gone. Switching now takes a lock and refuses if the seat moved underneath it, a half-finished swap is completed rather than compounded, and a session running on another account's token no longer reports its usage as the seat's. If you use one account, nothing here changes for you. ([#148](https://github.com/The-AIOS/aios/pull/148))

**Spawned and background sessions were starting with no context at all.** The mandatory context floor runs through `uv`, and `uv` insists on a cache directory that a sandboxed session may not write — so it stopped before running, produced nothing, and a floor that emits nothing is indistinguishable from one that worked. Every sandboxed session was affected; the ordinary interactive session never was, which is why it lasted. The prescribed command now sets its own cache location. ([#159](https://github.com/The-AIOS/aios/pull/159))

**New sessions now start on Claude Opus 5.5.** AIOS pins the model for its default rung rather than inheriting Claude Code's, so that spawned sessions keep the 1M-context model reliably. The side effect was that when Opus 5.5 shipped, every session AIOS launched stayed on Opus 5 — including for anyone who had already chosen 5.5 inside Claude Code, because the pin outranks that choice. It now names Opus 5.5, on macOS, Linux and Windows alike. To run a different model, `CLAUDE_MODEL` still overrides it exactly as before. ([#160](https://github.com/The-AIOS/aios/pull/160))

**If you keep an `antifragile.md`, sessions were starting without its rules.** The context floor hands every session a rule library's meta-pattern index, but it stopped reading at the index's first sub-heading — so an index written as `###` entries arrived as a heading with nothing under it. Measured on a live vault: 0 of 23 meta-patterns reached a session; now all 23 do. The floor also refuses, naming the file, when it cannot read one, instead of leaving it out silently. ([#163](https://github.com/The-AIOS/aios/pull/163))

**`/weekly-learnings` now measures its three pages instead of estimating them.** HTML has no pages, so a report that overflows reads fine on screen and only spills onto extra sheets when printed — one real report printed on six. After saving, it now renders the report, names any page that runs over and by how much, and trims only that page. `/learned` gets the same check, but moves overflow onto a new page rather than cutting insights. It needs Chrome or Chromium; without one it says the fit was not measured. ([#165](https://github.com/The-AIOS/aios/pull/165))

**Also closed:** a safety check in `/aios:update`'s reconcile that could *report* a broken run but not *stop* it. It had never fired ([#161](https://github.com/The-AIOS/aios/pull/161)). `FORTRESS.md`'s git-hook check looked in the wrong place, so it could report the hook missing when installed and installed when missing ([#163](https://github.com/The-AIOS/aios/pull/163)). And four places in the docs restated a number another file owns, or contradicted a neighbouring rule — each now points at its owner, and the README's map lists Security, Glossary and Contributing ([#164](https://github.com/The-AIOS/aios/pull/164)).

**And for anyone contributing:** the test suite largely could not run on Windows — 34 of 61 suites failed, 29 of them on that same `python3` stub — so a Windows contributor could not check their own work. Fixed across 28 suites. ([#147](https://github.com/The-AIOS/aios/pull/147))

**Action required:** one approval. Everything lands with your next `/aios:update`, and because the Opus 5.5 change is in the script that writes your shell profile, the update will **show you that change and ask before re-running it** — the only behavioural difference in it is that one model name. Open a new terminal afterwards for it to take effect. Two notes if a symptom sounds familiar: if your daily plan has felt an hour or two off and you mirror a work calendar into Google, that was the calendar fix, and it applies from your next `/aios:today`. **If you mirror a work calendar and its times still look off after updating**, set your timezone line — it's what lets the plan place mirrored events. Without it, every event renders exactly as it did before this release. It must be in `USER.md` → `## Sources` → `### General`, in exactly this shape — ``- Timezone: `America/Chicago` `` — without the template's italics, which mark the line as an unfilled placeholder. If a previous `/aios:update` on Windows reported a large number of framework files as drift, that was this bug — nothing was lost, and the next run shows you the real list.
...
**The levers you actually hold, model choice first.** `SECURITY.md` now lists them in the order that matters, and none of them is something AIOS switches on for you. The first is the one no permission setting can replace: **when a malicious document tells a session to paste a credential somewhere, the thing that declines is the model.** That behaviour differs between models, so which one holds your credentials is a live choice rather than a default — a spawned worker is always a Claude model, but the repo ships [`AGENTS.md`](./AGENTS.md) for other tools and the desktop app opens terminals where whatever CLI you run is what is actually running. The others: scope your connectors to what you need, set your autonomy levels deliberately, and **if you leave a session genuinely autonomous, point it only at sources you trust** — autonomy is fine and untrusted input is survivable, but the combination has nobody present to answer a prompt, and silence must never count as yes.
...
**Fixed — Windows, and this one could have cost you your context.** On Git for Windows, `ls` is aliased to `ls -F`, which appends `/` to a directory name. `/aios:update` derived its layer list from `ls`, so every entry in the denylist that protects your own content missed — and the completeness reconcile then compared **your** `vault/` against canonical's empty seed files and reported your entire declared and observed context as framework drift to pull. The command's own instruction for such a line is to apply it like a framework file. It exited 0, wrote nothing to stderr, and the lines were shaped exactly like real drift ([#141](https://github.com/The-AIOS/aios/issues/141), reported by an operator whose run stopped to ask why their own notes were being described as framework drift).

No name is derived from `ls` any more — it is a display tool that honours the caller's aliases, colours and classify flags, so it has no business producing values a script then matches on. And because a denylist can be syntactically perfect while matching nothing, both derivations now **assert that the exclusion happened** and abort if it did not, naming the folder that was not excluded. A related filter missed vault paths entirely: GNU diff quotes a path containing a space, and every vault path has one by design (`00 - notes`).
...
**Action required:** none, and **if you are on Windows, run `/aios:update` before your next sync** — a run from the older code can misreport your own context as framework drift. Both new files arrive on that run, which also stamps the date whether or not there is anything to pull. If you connect MCP servers beyond the bundled set, `mcps/_index.md` § *Connecting a server whose code AIOS does not ship* is the section worth reading first.
...
**New agent: `brochure-builder`.** AIOS had no way to make collateral — `deck-builder` makes presentations (projected, narrated), `infographic-builder` makes one visual page from a doc you already have, `design-md-author` authors a design system but never an artifact. A sell sheet, a service one-pager, a capability statement — the thing that gets forwarded to someone who then acts on it — had no owner.

It works from **whatever brand you actually have**, which is the part that makes it usable on the *first* brochure rather than the second: use design guidelines if they exist and **look before asking** (an existing brochure is a spec); derive them from a URL if you give one, reporting the palette and type back so you can correct them *before* rendering; or propose one direction from the content with a named rationale. Then it offers, once, to persist what you approved via `design-md-author`. It renders through the bundled PDF MCP rather than driving headless Chrome (which hangs), rasterises anything leaving your machine (Chrome's Type-3 fonts render badly in other viewers), and **never invents a client name, date, price, metric or testimonial** — a missing field is left out and reported, because collateral is the artifact most likely to be forwarded to someone who acts on it. It stages the file and stops; sending is your signature.

**And the loading rule now has exactly one home.** `CLAUDE.md` was shipping **two contradictory specs**: § Identity & Greeting said floor-then-fit while § II → Session Start still said *read every file* — so a session's context budget depended on which section it happened to read. § II is now a pointer, keeping the capability-recalibration check and *scan `antifragile.md` before executing commands*, which are the two things that genuinely belong at startup.

Two selector fixes came with it, both from measuring what had been assumed. **`antifragile.md` is a rule library, not a chronology** — an entry from four months ago binds exactly as hard as one from this week — so the floor now emits the meta-pattern index the file's own text says to read first, instead of five entry bodies. And **the tail of a file is not reliably its newest entry**: measured across nine observed files it was newest in only 5 of 8, so recency now comes from the date each entry carries. Net effect on the floor: **smaller and more useful at once.**
...
**And `install-git-hooks` now normalizes every hook `.gitattributes` protects** — with its `chmod` deliberately narrower than its normalization. The widened loop first chmodded *every* shebang file in `hooks/`, which changed the mode of three library scripts (`pipeline-executor.py`, `route-insight.py`, `guard-venture-mount.py`) on the very next sync: three mode-only `644→755` diffs, zero content changed, sitting in the operator's tree for `aios-commit --vault` to sweep into a session commit as if they were work. The repo distinguishes 755 (invoked by name) from 644 (run through an interpreter) on purpose, and nothing was gained by flattening it — a 644 `.py` called as `python3 hooks/x.py` runs fine. So `chmod` now covers only what is invoked by name (`hooks/git/*` plus the extensionless tools), while CRLF normalization stays broad, since it only rewrites a file that actually contains a `CR` and therefore never dirties a clean tree. Caught by running `/aios:update` on a live vault immediately after shipping it. Today's earlier fix covered `hooks/git/*` and `aios-commit` while `.gitattributes` declares `hooks/aios-*` too — leaving `aios-snapshot` (mandatory in the Session End ritual) and `aios-star-check` (`/aios:update` Step 6.9) CRLF-exposed on Windows, the one platform the fix exists for. The globs now mirror the declaration, and the suite extracts the installer's **own** globs to check it, after an earlier version carrying its own glob list stayed green when the installer was narrowed. Found by finally reading a roadmap row that had described this exact gap since 2026-08-13.
...
- **Your session can now recommend the birth of a roadmap layer — at the right moment, not by default.** The framework has always shipped the opt-in keyed roadmap (`templates/aios/roadmap-template.md` + the ship-time truth-flip contract), but nothing said *when*. Grammar § 7 defines the graduation signals — project notes repeatedly flagging the line-count ladder because they carry prioritized cross-project backlogs; one initiative's tasks fragmented across 3+ notes; carries bouncing between notes; cross-everything questions no file can answer. Two or more firing → your session surfaces the offer: one keyed roadmap file first, and only when *that* grows heavy, the birth of `00 - notes/roadmaps/` (the folder doesn't exist until this moment) with one map per family/venture, an `_index.md` compass, and each project note wired to its map via a `Roadmap` row. Projects stay dashboards; the maps own DONE-vs-OPEN. Declining is a fine answer and won't be re-proposed for 30+ days.
...
- **Restructure boldly without losing a byte.** Say *"dissolve this folder"* / *"split this note"* / *"rename this everywhere"* — or accept the graduation offer above — and your session loads the **`vault-migrations`** skill: dry-run first with a printed audit, inventory verification (file/key/line accounting — "probably nothing was dropped" is not a verification), no silent overwrites, one commit per verified stage, and the field-tested gotchas that each cost a real migration a real bug. The graduation migration (scattered backlogs → keyed maps, where losing one row means a commitment silently vanishing) is the skill's canonical worked example.
- **Get structure drift surfaced before it taxes you.** `/aios:housekeeping` Bucket 21 (now *File placement drift + structure grammar*) audits the vault against the grammar's checklist — mixed retirement names, archive shelves inside record folders, archived notes still carrying live obligations, alias-less renames, graduation signals — as **proposals only**, never auto-moves.

### Components (for the record)

- `templates/aios/vault-grammar.md` — NEW (reference template, read in place; § 7 = the roadmap-layer graduation; registered in `templates/_index.md`)
...
- `plugins/aios/commands/housekeeping.md` — Bucket 21 upgraded (placement scan unchanged; grammar + graduation scans added; propose-only contract unchanged)
...
3. **Nothing to run for the wiring (#82)** — `/aios:update` lands the two command files and syncs them to the plugin cache automatically. Check-then-act: if your vault has no live `type: roadmap` files, nothing changes for you; if it has, your next `/aios:close-day` starts the loop (first board generation + ledger sweep) and the morning after, `## The Board` appears in your daily note. **One caveat, found live-dogfooding this entry:** the harness snapshots command files at **session start**, so a session opened before this sync renders the OLD `/today`/`/close-day` templates — even for commands never invoked in that session. Check-then-act: sessions started after the sync are fine as-is; for an already-open session, either restart it or have it Read the command file from disk and follow that (the same move `/aios:update` § 2.5 uses on itself). Restart, as always, is the last step.
...
>
> **If you read one section, read § What you need to do at the end.** Almost nothing here needs you — `/aios:update` lands the files and re-runs the installers. There are three exceptions and they are short.

---

### `voice-gate` — grade the draft, keep the voice

A bundled skill that checks anything about to ship in your name — essay, post, chapter, email, proposal, deck copy — for the constructions that read as machine-written, and **reports rather than rewrites**. It scores three things: how often a tell appears **against your own baseline**, whether tells cluster inside a paragraph, and **where** they sit. Position is the point: a reversal in a title is the reader's whole impression of your voice; the same sentence mid-paragraph is invisible and usually doing honest work.

It deliberately refuses to ban anything. *"It's not X, it's Y"* is the highest-signal tell in generated prose **and** the spine of plenty of real writers' arguments, so it is governed by a budget — never twice in one paragraph, earned in quotable positions, free in body prose. Run a banned list against a writer who argues by reframe and you delete the thesis along with the tell.

It calibrates from your own published work: it reads `context/declared/personal_voice.md`, your recorded writing corrections, and a sample of what you have actually published, then measures **drift from your norm**. Two operators can legitimately sit several times apart. Wired into five agents that ship voice outward — `content-writer`, `email-drafter`, `report-drafter`, `deck-builder`, `sales-proposal-writer`.

**Read this before relying on it:** it does **not** defeat AI detectors, and says so in its own text. What protects a writer who is genuinely writing is **provenance** — drafts, version history, recordings — not sanded prose. It also ships false-positive guards, because detector-style scoring lands hardest on second-language and neurodivergent writers: simpler constructions, structured style, polish, a lone em dash and curly quotes are each explicitly **not** evidence. Treat any score as a lead, never a verdict.

*Pattern taxonomy adapted from [Wikipedia's "Signs of AI writing"](https://en.wikipedia.org/wiki/Wikipedia:Signs_of_AI_writing) (WikiProject AI Cleanup). No third-party code or prose vendored.*

### `MODEL-ROUTING.md` — which Claude to spend, and where the non-Claude boundary is

On the same trivial prompt, the top and bottom of the model ladder differ by roughly **22× in cost** — measured, not estimated. A new root doc answers *"which model should do this task?"*, and `spawn --tier` grew from two rungs to four:

- **`frontier`** — hardest problems: long-running autonomous agents, code migration, multi-step reasoning, full autonomy
- **`judgment`** *(the default, unchanged)* — reasoning-intensive work: legal, financial, research, and production coding
- **`scale`** — general-purpose workloads at volume
- **`fast`** — high-frequency, latency-sensitive tasks; sub-agents inside an orchestration

Pick by the **shape** of the work, not its importance — "important" is what tempts you to the top rung for a file sweep. Omitting `--tier` still gives you `judgment`, deliberately: under-powering a reasoning task fails **silently**, since you get an answer, it is merely worse, and nothing tells you.

**`--tier mechanical` still works and still resolves to the model it always did**, byte-identical. It predates the ladder and is live in habits, routines and scripts; changing what an existing flag resolves to would change the cost *and the answers* of work already running. `scale` is its successor for new work, and nothing forces a migration.

**Verify a model id rather than trusting it.** An id Claude Code does not recognise **does not fail loudly** — it prints `[claude-code:unrecognized_model]` and then bills **zero tokens**, so *"the spawn worked"* is not evidence on its own. The doc carries the one-line probe that can actually fail, plus the trap inside it: the `modelUsage` field echoes the id you *asked for*, so a bogus id appears there verbatim and looks confirmed. Cost and token counts are the discriminating signal.

**`hooks/openrouter.py` — call a non-Claude model as a tool.** Text in, text out, stdlib-only so it runs from a routine on a fresh machine with nothing to install. Two uses, one file: a cheap lane for bulk text, and an **independent judge** — because a model cannot dock points for a tendency it *shares*, so scoring Claude-authored prose with a Claude model is a family grading itself, and the bias is invisible by construction. It resolves `OPENROUTER_API_KEY` → OpenRouter, else `GEMINI_API_KEY` → Gemini direct, else **refuses and names both paths and both files**. Never a silent no-op: a scorer that quietly returns nothing reads exactly like a scorer that found nothing wrong.

**Three boundaries, stated as rules rather than left to be discovered:**

1. **Never put a third-party proxy in the request path.** There is a class of tool that makes Claude Code believe it is talking to Anthropic while routing elsewhere. They work, which is the problem: every session holds live Gmail, Drive and Slack credentials plus a private vault. **This is a no.** Reaching for one is the natural next step after wanting a cheaper model, which is exactly why it is named.
2. **A spawned worker is always a Claude model.** `spawn` passes `--model` to the `claude` binary, so a non-Claude model is *called by* a session and can never *be* one. That keeps other families away from the MCP tool catalog and vault writes, where reported measurements show smaller models degrading sharply as the catalog grows.
3. **The rail sends your content off-machine to a third party.** Not a caveat — the mechanism, and the one thing the rest of AIOS avoids. It is **opt-in and inert until you create a key file.** Pass it prose drafts; keep client material, NDA'd content, financial records and personal context on your own machine.

`/aios:housekeeping` gained **Bucket 27** to sweep for same-family judges in your own `custom/` surfaces — report-only, and it distinguishes *reading* an artifact Claude did not write (fine; the framework does it) from *grading* one Claude wrote (the finding).

### The observation buffer holds two classes with opposite exits — and one could never leave

`session-insights.md` was one buffer with one promotion rule: *an entry leaves when a second independent sighting confirms it.* That is right for observations about **you** and structurally wrong for observations about **the system** — because for a system finding, **a second sighting is not confirmation, it is evidence the fix never landed.**

So the better AIOS gets at fixing itself, the more its findings are one-offs, and one-offs can never satisfy that exit condition. That is why raising the cap never held. Intake also scales with sessions-per-day while the promotion rule does not, which is why this is invisible on a single-session vault and unavoidable on a fleet.

Every entry now declares its class on the line under its title:

```
`class: behavioural` · `first-seen: 2026-09-04` · `route: patterns.md`
```

- **`behavioural`** (about you) — **unchanged.** Emerging → Reinforced on a second sighting → routed. One sighting might be noise; waiting is correct and costs nothing.
- **`method`** (about the system — a command, a hook, an agent's own behaviour) — **never waits.** It needs a rule general enough to fire outside the incident, then exits in that same close three ways: **route** it, **fold** it into an entry it sharpens, or **drop** it because an existing rule already covered this and applying that rule harder was the real answer. **Most fold or drop — that is success, not attrition.**

**If you can state the rule, route it now.** Sessions already do this when confident, which is exactly why what accumulated was the residue nobody was confident enough to route directly. A buffer holding only unconfident method findings is a confidence-holding pen, not a queue.

**Measure it instead of reading it — `hooks/buffer-status.py`.** Reading the buffer in full costs ~13–22k tokens, which is why its cap and clock drifted while they were enforced by impression. This costs a couple of hundred: counts by class, what is over cap, what states no route target, what is past the 30-day clock. **Reads only, never edits.** Exit `0` within contract · `1` action needed · `2` **could not measure** — an unparseable file fails loudly rather than reporting a healthy zero. `/close-day` calls it now.

**Entries with no `class:` line are unclassified, never assumed.** No migration script, nothing auto-labelled, nothing auto-deleted.

*Reported with machine-derived figures from a vault running a multi-session fleet — 34 entries against a raised cap of 25, 23 under a week old, zero near-duplicates across 561 pairwise comparisons, 28 of 34 system-class. Two of those reproduced independently on a second vault that had never seen the report; a third did not, and is recorded as vault-specific rather than general. The write-path primitive remains open for the reporter, who offered to build it against a settled schema.*

### The containment ladder — six rungs, and the one door the walls document never mentioned

`FORTRESS.md` described a two-machine build costing ~$600 and a weekend, and it was the **only** containment guidance the framework had. So the honest answer to *"how safe is my setup?"* was either *"buy a Mac mini"* or nothing. It now opens with a ladder you are already standing on:
...
- **Rung 4 — a cheap always-on box** (~€5–10/month). The rung most people skip straight past and usually the right one: it buys the property that actually matters — *something that stays running when the laptop closes* — without hardware on your desk.
- **Rung 5 — the two-machine fortress.** The rest of the document, unchanged.

**Most of the real risk reduction is rungs 1 and 2, and neither costs money.** A second machine contains the blast radius of an agent that misbehaves; it does nothing about a key exported in a shell rc. Buying hardware first is the satisfying move and the wrong order. The doc names the rung by **what is true, not what is aspired to** — an operator with a two-machine fortress and a key in `~/.zshrc` is on rung 1, because containment is the weakest link and not the priciest component.

**Ask a session *"which containment rung am I on?"*** and it runs the doc's probe block. `/aios:housekeeping` re-checks on cadence as **Bucket 28** — read-only, reporting *regressions* rather than a score, because containment degrades quietly: a key exported during a debugging session and never moved back, a hook that stopped being installed after a machine rebuild.

**And the agent bus is finally in the document.** Six defensive layers described the walls; nothing described the door. Agents coordinate across machines by **writing files** — `spawn`, `send` and `kill` are each a JSON file dropped into `~/.aios/spawn-inbox/`, fulfilled natively by whatever surface runs on the far machine. It is **pull-based**: the receiving machine opens no port and runs no listener, so it passes *through* the firewall posture rather than against it, and there is no broker to fail. The transport being a file makes the substrate pluggable by construction. The section states the **zero inbound surface** design invariant, says why a file bus rather than the built-in cross-session relay (no cloud transit, no plan gate, substrate-pluggable), and states plainly what is missing: **a request file carries no authentication and no requester identity, so do not share a spawn-inbox** until requests are signed and verified.

### `shipping-a-saas` — what to build first, and the rung everyone gets wrong

A skill carrying the **order** a product gets built in, and the defaults that are cheap on day one and near-impossible to retrofit.
...
**Rung 2 is second on purpose.** The instinct is to build admin tooling last, because no customer asks for it — and the consequence is that for the whole period *before* it exists, which is the period when everything is broken, nobody can see inside the system. Every support question becomes a hand-written database query and every demo is either fake or terrifying. Nobody stops to add it later. **Auth is rung 3, not rung 1**: an admin view behind a single environment variable is enough to make rungs 1–2 real, and a product with no users does not need a password-reset flow.

**Defaults that are hard to retrofit:** deterministic seed data with **no random generators** (*a test whose input you cannot reconstruct is not a test, it is an anecdote*) · non-sequential ids on anything a client can see · one command from a fresh clone to a running app **with data in it** · one place for the data model · integer minor units for money and UTC everywhere, decided on day one.

**PR discipline:** rebase before you *build* rather than before you merge · one logical change per PR · say what would break if the change is wrong · **require that CI could actually fail**, by mutating the code and seeing the red line · merge shared-file PRs serially · scope your commits when other agents write the same tree.

**It recommends a stack, and defers to yours.** For an operator with no existing opinion: Elixir + Phoenix API-only · React + TypeScript + Vite · PostgreSQL, Redis, Meilisearch and a DB-backed job queue · Nix flakes for a one-command environment · a push-to-deploy platform for most products, bare metal with declarative OS config when the economics or control genuinely win. **One coherent stack rather than a menu**, because mixing halves of two stacks gets you the integration problems of both. Two caveats ship with it: **Nix has a real learning curve** — timebox the spike and keep a fallback rather than yak-shaving — and a recommended stack is **not a migration order** for products you already run.

**Your own declaration outranks it.** If `context/declared/coding_style.md` or `coding-practices.md` exists, **it wins** — the skill reads yours first and treats the recommendation as already answered. When the two disagree that is not drift: **a deviation is a decision**, recorded with its reason. Wired into `technical-cofounder`, `code-reviewer` and `security-engineer`.

### A seventh agent bundle — `commerce/`, holding one agent

`commerce-agent-architect` designs, builds and hardens a **shopping agent** (buyers: search, comparison, cart, order assembly) or a **merchant agent** (operators: sales insight, inventory, pricing, promotions). One model in a standard agent loop, skills for the long tail, tools onto the systems you already run, and memory in a database rather than accumulated in context.

**The load-bearing rule:** *model intelligence decides what to do; the harness decides what is allowed.* Reasoning and tool selection are the model's job — **policy enforcement, money movement, discount ceilings and inventory decrements are code, not instructions in a prompt.** A prompt is a request; a guard is a guarantee. Same principle as the observation buffer above, and it fails the same way when ignored: silently, except here while spending someone's money.

**Nothing is vendored, deliberately.** Anthropic ships a full reference implementation at [`anthropics/commerce-agents`](https://github.com/anthropics/commerce-agents) (Apache-2.0) — working shopping and merchant codebases, four worked verticals, and a `commerce-builder` plugin with four commands and six skills. That is **6.7 MB whose whole value is staying current with its authors**; a copy here would drift and duplicate what one install line reaches. The agent points at it and stays the entry point, the same shape `lawyer` already uses for the official legal suite.

**And one agent rather than three**, because the guidance it draws from explicitly rejects splitting the conversational path into cooperating sub-agents. Shipping `shopping-agent` and `merchant-agent` as separate bundled workers would contradict the very argument they would be implementing; they are modes of one architecture.

### Housekeeping and CI fixes worth knowing about

- **`/aios:housekeeping` said "all 23 buckets — no skipping" while 27 were defined.** Buckets 24–26 shipped without that number being bumped, so a session following the instruction literally stopped at 23 and **silently skipped three working buckets** — memory-pressure channeling, spawned-output placement drift, and antifragile fix-verification. The count is now derived from what Phase 1 actually contains rather than restated. **If you have run housekeeping since those shipped, they were probably not in your packet; the next run will include them.**
- **The framework-purity CI check globbed the filesystem rather than asking git**, so a gitignored build artifact read as a company-namespaced folder and failed an outside contribution whose own suite passed. It now enumerates tracked directories, which makes the whole class impossible.
...
*Four more surfaced across two CI runs, every one of them the entry's own thesis pointed back at its author — and each was invisible on the authoring machine specifically.*

- *The `.ps1` carried **553 non-ASCII bytes with no BOM**, which PS 5.1 corrupts at parse time. The existing guard caught it instantly; the local pre-flight that had cleared it used `grep`, which reports **zero matches** on a file it decides is binary — so the check meant to prevent this returned a false negative, and only a byte count was honest.*
- *`"failed=$r:…"` is a **parse error**, not a runtime one: PowerShell reads `$r:` as a scope qualifier (the syntax of `$env:PATH`), so the entire file fails to load. Only a Windows runner could see it, which means anyone without one authors blind — so there is now a **static** check on the fast Ubuntu job that catches the class in seconds. Its first draft flagged its own explanatory comment, and the control meant to prove it worked was then measuring that comment rather than the code; it is comment-aware now, verified to fire on the real defect and to stay quiet on legitimate `$env:`/`$global:` and on prose describing the bug.*
...
**Two MCP servers were writing into a vault that did not exist.** A sweep for siblings of the bug above found `pdf-generator-mcp` and `nano-banana-mcp` both defaulting their output to the home directory joined to a literal `aios` segment — then calling `mkdir(parents=True)`. Unlike the executor, `~/aios` **is** the documented install path, so this worked for everyone whose symlink was in place. Where it wasn't, the failure was nastier than a missing directory: the `mkdir` did not fail, it **created** a phantom `~/aios/vault/03 - export/generated/` tree and wrote your PDFs and generated images into it, reporting success each time. Both now derive the vault from the server file's own location, so output follows the vault the server ships with and the symlink is no longer load-bearing. **This one needs a restart to take effect** — see Action required.

**What you can now do (MCP outputs):**
- **Find your generated files where you expect them.** `mcp__pdf-generator__html_to_pdf` / `markdown_to_pdf` and `mcp__nano-banana__generate_image` with no `output_path` now land in *your* vault's `03 - export/generated/` and `02 - assets/generated/`. If you ever generated a PDF that you could not find afterwards, check `~/aios/vault/` for a stray tree — that is where it went, and those files are still there.
- **Stop relying on the symlink for this.** Framework *code* that can locate itself now does; `~/aios` remains the documented convention for commands and docs, which is a different thing from code hardcoding it.

**Why this survived a CI guard that exists for it.** The validation suite has checked for references to the retired vault path since the extraction — but it grepped for the path as **text** (`~/obsidian`, `$HOME/obsidian`, `/Users/*/obsidian`). The real hardcode was *assembled programmatically* from the home directory, so not one pattern matched: the guard reported "no hardcoded paths" about a state it had never examined, and stayed green for two months over three live occurrences. A second check now covers the constructed form in `.py`/`.sh`/`.ps1`, and a third requires framework *code* to self-locate rather than hardcode the `~/aios` install path — narrower on purpose, so commands and docs may keep referencing `~/aios` (it is the documented convention) while code that can find itself must. **Both new guards were run against the buggy files before shipping and confirmed to flag them**; a guard that passes on the bug it exists for is worse than no guard, because it also certifies safety. The lesson generalises past this path: **a check that knows only one spelling of the thing it forbids is not a check.**

**If you have not filled in `USER.md` yet, here is what you will see.** Its `*EXAMPLE ONLY*` lines do match the parser's substring checks, so internally every source looks "configured" while the actual values stay empty. The visible outcome is still correct, because each source is additionally gated on its credentials existing: the run skips them and the `## Pipeline Status` section at the end of the output names each one — `⏭️ calendar: not configured`. So an unconfigured vault reports itself accurately; fill in your real values under `## Sources` and the skips turn into data. (Tightening the parser to ignore example lines would be internal tidiness, not a behavior fix — deliberately not bundled.)

### The Google Workspace MCP had three runtimes, and none of them agreed

> **What this delivers.** Three framework surfaces each gave a different answer about how this server actually runs: `SETUP.md` registered a **vendored copy** in the repo, `mcps/setup.sh`'s own summary advertised **`uvx`** (PyPI), and `mcps/_index.md` called it **"vendored."** Whichever one you followed, something else was lying. The vendored copy had meanwhile frozen at **1.15.0** against an upstream now at **1.22.2** — seven minor versions including several OAuth fixes — and it imported a `gappsscript` module that was never copied in, so enabling that service would have crashed. It went unnoticed because the machine where the vendoring was maintained had drifted to `uvx` and never ran the copy. On top of that, `setup.sh` built a **~196 MB venv** for every operator, for a runtime their own registration didn't point at. The source was 1.6 MB; the venv was all the rest.

**What you can now do:**
- **Get one true answer about what's running.** The framework standardizes on **`uvx workspace-mcp`** — resolved from PyPI when Claude launches it. Nothing to build, no second copy to drift, and you track upstream (which ships every few days) instead of a frozen snapshot. The folder keeps only what is genuinely ours: your OAuth template, the Cloud Console walkthrough, the troubleshooting recipe, and `make_auth_link.py`.
- **Actually complete the Google setup as a new operator.** This is a real bug fix, not a cleanup: SETUP.md claimed OAuth credentials were *"bundled in the repo at `mcps/google-workspace-mcp/oauth.json`"* and then read that path. That file is **gitignored** — it's a per-person secret and only ever existed on machines that had created one — so every fresh operator hit `FileNotFoundError` and stopped. The step now routes through `oauth.json.template` plus the existing console walkthrough. Verified both directions: the old instructions raise `FileNotFoundError`, the new ones resolve.
- **Stop the two-registrations-that-drift trap.** `claude mcp add` defaults to **local** scope, which is *per-directory* — a second registration made from another folder is an independent copy. Two of them here diverged exactly that way (one grew Gmail/Contacts/Forms, the other didn't) and no surface reported the difference. SETUP.md and the README now say: register once from `~/aios`, or use a single `--scope user` registration if you want it everywhere.
- **Turn on Google Chat if you need it.** Upstream ships Chat tools — list spaces, read/search/send messages, reactions, attachments — and the default permission list omits them. Add `chat:full` (or `chat:readonly`). It requests new OAuth scopes, so expect a fresh consent prompt. Caveat worth testing before depending on it: Google restricts *sending* to some space types when using user credentials rather than a Chat app.

**Why this is not pinned, deliberately.** The old `UPSTREAM.md` chose vendoring to *"pin a known-good, audited version"* — a sound goal, and precisely how it ended up on a stale, broken 1.15.0. **A pin with no bump discipline is not a pin, it's a freeze** — and it's worse than floating, because it also feels deliberate. Nothing in the framework reliably raised that version, so the guarantee was fictional. Floating on an actively-maintained upstream has empirically been the lower risk (this vault ran unpinned across ~25 releases without incident). If you want determinism anyway — a fortress machine, a shared team install, recovering from a bad release — `UPSTREAM.md` documents pinning via `uvx workspace-mcp@1.22.2`, with the explicit condition that you own the bump.

**Action required — nothing is forced, and nothing breaks. Check which runtime you're on (30 seconds).**

> ⚠️ **First, if `/aios:update` asks whether to delete files under `mcps/google-workspace-mcp/`: answer KEEP** — unless you have already done step 3 below. This update removes the vendored server from *canonical*, and `/aios:update` never auto-deletes your local copy (it asks, and the default is keep). Answering "delete" while your registration still points at that tree is the one way to break your Google MCP here, so decline until you have re-registered.

1. **See what you're actually running:** `claude mcp get google-workspace` (or grep your `~/.claude.json`).
2. **If it says `uvx workspace-mcp`** — you are already on the standard path. Nothing to do. Optionally reclaim ~196 MB: `rm -rf ~/aios/mcps/google-workspace-mcp/.venv` (it was never your runtime).
3. **If it says `uv run --directory …/google-workspace-mcp` or `…/.venv/bin/workspace-mcp`** — you are running the frozen 1.15.0. **It keeps working; your local files are untouched.** Migrating is recommended, not urgent: re-register with `-- uvx workspace-mcp --single-user --permissions …` (full command in `SETUP.md` → Google Workspace MCP), confirm your calendar/mail still answer, *then* delete the leftover code + `.venv`.
4. **Fresh install?** Nothing to undo — just follow `SETUP.md`, which now works. If you tried before today and got `FileNotFoundError` on `oauth.json`, that was this bug, not you.
5. **Restart Claude Code last.** An already-running MCP server keeps its old command line until the process restarts; re-registering does not reload it in place.
...
5. **If you use the `pdf-generator` or `nano-banana` MCPs, check for a phantom tree, then restart LAST.** Run `ls ~/aios/vault/` — if `~/aios` is a real directory rather than your vault's symlink and it contains a `vault/` with generated files, those are outputs that never reached your vault; move them where you want them and remove the stray tree. Then **restart Claude Code** so the MCP servers reload — unlike the executor (plain library code the next `/today` picks up on its own), a running MCP server keeps its old code until the process restarts. Nothing else in this entry needs a restart.
...
3. **If you work in the IDE, update AIOS Glass to `0.5.2` — `/aios:update` does NOT deliver it.** Glass ships through **Open VSX**, on a separate release line from the framework, so syncing the vault leaves a stale extension in place. `0.5.2` carries the command-bus half of the long-`send` fix; **on an older Glass a long prompt is still silently cut**, and the sender sees a success either way — which is precisely the failure the fix exists to remove, so a half-updated pair is worse than an obviously broken one.
   - **Check what is actually RUNNING, not what is downloaded.** Extensions view → *AIOS Glass* → the version shown. This distinction is not pedantic: Open VSX auto-update **downloads** the new build beside the old one and keeps running the old one until the window reloads — a real machine tonight had `0.5.1` *and* `0.5.2` installed side by side.
   - **To update:** let Open VSX auto-update fetch it (or Extensions view → search *AIOS Glass* → **Update**), then **Reload Window** — `⇧⌘P` → *Developer: Reload Window*. The reload is the step people skip, and skipping it is indistinguishable from the update not having happened.
   - **Already on `0.5.2` and reloaded?** No action.
4. **Only if you installed a build BEFORE 0.7.1** (the pill did not exist yet, so the update arrives as a **macOS notification**): **quit the app → wait a few seconds → open it again.** ⚠️ **The wait is not optional and it is not a formality.** The swap happens *after* the process exits and it copies ~123 MB, so quitting and immediately reopening lands you back on the old version, still notified — reopen too fast and it looks like a broken update when it is simply unfinished. If you already did that, quit once more and give it a moment. From 0.7.1 onward the pill removes this dance entirely.
