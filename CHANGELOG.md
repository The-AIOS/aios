...
> **What this delivers.** A new bundled skill that checks anything about to ship in your name — essay, post, chapter, email, proposal, deck copy — for the constructions that read as machine-written, and **reports rather than rewrites**. It scores three things: how often a tell appears (against *your* baseline, not a universal number), whether tells cluster inside a paragraph, and **where** they sit. Position is the point: a reversal in a title is the reader's whole impression of your voice; the same sentence mid-paragraph is invisible and usually doing honest work.

**What you can now do:**

- **Ask "does this sound AI-written?" and get a location, not a verdict.** Point the skill at a draft and it returns a density figure next to your own baseline, every paragraph containing two or more tells, and every instance sitting in an exposed zone — title, subtitle, hook, pull-quote, closing line, back cover — quoted in full. The exposed-zone list is the one you actually act on; it is usually three or four lines.
- **Keep the moves that are genuinely yours.** The skill deliberately refuses to ban anything. *"It's not X, it's Y"* is the highest-signal tell in generated prose **and** the spine of plenty of real writers' arguments, so it is governed by a budget — never twice in one paragraph, earned in quotable positions, free in body prose. Run a banned list against a writer who argues by reframe and you delete the thesis along with the tell.
- **Calibrate the budget from your own published work.** Before judging a draft, the skill reads `context/declared/personal_voice.md`, your recorded writing corrections, and a sample of what you have actually published, then measures **drift from your norm**. Two operators can legitimately sit several times apart. If you supply a writing sample, it overrides every default in the skill, including the guidance on dashes.
- **Get it automatically wherever voice ships outward.** Wired into five bundled agents: `content-writer` (including its pre-delivery checklist, so the report ships with the draft), `email-drafter` (subject line + opener), `report-drafter` (executive summary + section openers — reports drift fastest because the form is already formulaic), `deck-builder` (slide copy is all exposed zone; presenter notes are left alone, since spoken register is not written register), and `sales-proposal-writer` (cover letter + summary).
- **Measure authorship directly when a piece came from a recording.** An optional traceability mode classifies each sentence against the source transcript as verbatim, edited, or composed, and reports what fraction is genuinely yours. Published measurement puts the readable-as-human boundary near 85% of the author's own wording, and finds it behaves as a threshold rather than a gradient — roughly one sentence in eight can be model-composed, which covers joins and transitions and not a section. Two rules carry that mode: never tidy the transcript first (the disfluencies are the voice, and a clean-up pass converts your prose into model prose before drafting starts), and hand composed sentences back to be re-said rather than improved.

**What it explicitly does not do — read this before you rely on it.** It does **not** defeat AI detectors, and the skill says so in its own text. Modern classifiers are trained adversarially against this whole category and catch the large majority of "humanized" output; the documented paradox is that the more fluent such output gets, the more detectable it becomes. What protects a writer who is genuinely writing is **provenance** — drafts, version history, recordings, notes — not sanded prose. The skill also ships false-positive guards, because detector-style scoring lands hardest on second-language and neurodivergent writers: simpler constructions, structured or repetitive style, polish, a lone em dash and curly quotes are each explicitly **not** evidence. Treat any score as a lead, never a verdict.

**Action required — none.** The skill auto-loads on match like every other; the five agents pick it up on their next run. If you keep a personal writing protocol or pre-publish checklist in your own vault, that is the one place worth a manual edit — add the gate as a step before your existing voice pass, so it filters mechanically and your pass spends its attention on judgment. If you had already built something similar under `skills/custom/`, verify byte-difference before removing yours; the bundled one is a different design and may not be a drop-in replacement.

*Pattern taxonomy adapted from [Wikipedia's "Signs of AI writing"](https://en.wikipedia.org/wiki/Wikipedia:Signs_of_AI_writing) (WikiProject AI Cleanup). Two open-source projects are credited as prior art in the skill — one for the rewrite-oriented approach this deliberately diverges from, one for the traceability threshold. No third-party code or prose is vendored.*

### `MODEL-ROUTING.md` + `--tier frontier|scale|fast` — stop paying frontier rates for a file sweep

> **What this delivers.** A new root doc answering *"which model should do this task?"*, four `spawn --tier` rungs instead of two, and a sanctioned way to call a **non-Claude** model as a tool when you need a different family. On the same trivial prompt, the top and bottom rungs differ by roughly **22× in cost** — measured, not estimated — so the rung you pick is real money on work that runs daily.

**What you can now do:**

- **Pick a rung by the shape of the work.** `spawn --tier fast` for high-frequency, latency-sensitive work and sub-agents · `scale` for general work at volume · `judgment` (the default, unchanged) for reasoning-intensive work — legal, financial, research — and production coding · `frontier` for the hardest long-running autonomous work and code migration. Windows: `-Tier fast`. The rung→model table lives in `MODEL-ROUTING.md` and **deliberately nowhere else**, because a model id restated in a second file is a fact that goes stale silently.
- **Keep using `--tier mechanical`.** It predates the ladder and still resolves to exactly the model it always did, byte-identical. It is live in habits, routines and scripts, and changing what an existing flag resolves to would change the cost *and* the answers of work already running. `scale` is its successor for new work; nothing forces a migration. (Re-verified in passing: the model `mechanical` has been pinned to is still a real id, so it has never been silently falling back to the frontier — worth checking rather than assuming, since it would have been invisible.)
- **Verify a model id instead of trusting it.** An id Claude Code does not recognise **does not fail loudly** — it prints `[claude-code:unrecognized_model]` and then bills zero tokens, which makes *"the spawn worked"* worthless as evidence on its own. The doc carries the one-line probe that can actually fail, and the trap inside it: the `modelUsage` field echoes the id you *asked for*, so a bogus id appears there verbatim and looks confirmed. Cost and token counts are the discriminating signal. Every id shipped here was checked this way against a deliberately-bogus control.
- **Call a non-Claude model as a tool — `hooks/openrouter.py`.** Text in, text out, stdlib-only so it runs from a routine on a fresh machine with nothing to install. It resolves `OPENROUTER_API_KEY` → OpenRouter (one key reaches hundreds of models), else `GEMINI_API_KEY` → Gemini direct — that fallback exists so the rail works before anyone funds a new account, since a Gemini key is already present in many installs for the image MCP. With neither, it **refuses and names both paths and both files**, rather than no-opping: a scorer that quietly returns nothing reads exactly like a scorer that found nothing wrong. `--check` reports what is wired without making a call.
- **Get an independent judge.** A model cannot dock points for a systematic tendency it *shares*, so scoring Claude-authored prose with a Claude model is a family grading itself, and the bias is invisible by construction. The same mistake sits in public: a leading creative-writing leaderboard judged by a Claude model ranks a Claude model first, while an independent benchmark judged by professional human writers over thousands of blind pairwise comparisons produces a different winner. Same question, different ruler, different answer. `/aios:housekeeping` gained **Bucket 27** to sweep for it — report-only, and it distinguishes *reading* an artifact Claude did not write (fine, and canonical does it) from *grading* one Claude wrote (the finding).

**Three boundaries, stated as rules rather than left to be discovered:**

1. **Never put a third-party proxy in the request path.** There is a class of tool that makes Claude Code believe it is talking to Anthropic while routing elsewhere. They work, which is the problem: every session holds live Gmail, Drive and Slack credentials plus a private vault, and such a proxy sits in front of all of it. **This is a no in canonical AIOS.** Reaching for one is the natural next step after wanting a cheaper model, which is exactly why it is named here.
2. **A spawned worker is always a Claude model.** `spawn` passes `--model` to the `claude` binary, which runs Claude models only — so a non-Claude model is *called by* a session and can never *be* one. That is the right shape, not a limitation to route around: it keeps other families away from the MCP tool catalog and vault writes, which is where reported function-calling measurements show small and open models degrading sharply as the catalog grows. AIOS sessions carry a large catalog.
3. **The rail sends your content off-machine to a third party.** Not a caveat — the mechanism, and the one thing the rest of AIOS is built to avoid. So it is **opt-in and silent until configured**. Prose drafts are usually fine to pass it. Client material, NDA'd content, financial records and personal context are not. Before reaching for it, check whether a *lower rung of the same ladder* or a **deterministic counter** solves the problem — the `voice-gate` skill above is the argument that counting beats judging whenever a countable proxy exists, and it needs no cross-family call at all.

**Also fixed, and worth knowing if you run `/aios:housekeeping`:** its instructions said *"gather evidence across all 23 buckets — no skipping"* while **27** buckets were defined and live. Buckets 24 through 26 — memory-pressure channeling, spawned-output placement drift, and antifragile fix-verification — had shipped without that number being bumped, so a session following the instruction literally stopped at 23 and skipped three working buckets, silently. The count is now derived from what Phase 1 actually contains instead of being restated, which is the same *one-bound-one-home* discipline the framework applies to its own thresholds. If you have run housekeeping since those buckets shipped, they were probably not part of your packet — the next run will include them.

**Also fixed — CI:** the framework-purity check globbed the **filesystem** rather than asking git, so a gitignored build artifact (`hooks/__pycache__/`) read as a company-namespaced folder. It went red on an outside contribution whose own suite passed 10/10: that PR was simply the first CI step in the repo's history to execute a Python hook, and a later step in the same job then read the bytecode directory Python had just written. Now it enumerates **tracked** directories, which makes the whole class impossible — a future artifact in any layer is invisible to the check, and no allowlist has to grow to accommodate one.
...
**It carries a recommended stack — and defers to yours.** For an operator with no existing opinion: Elixir + Phoenix API-only · React + TypeScript + Vite · PostgreSQL, Redis, Meilisearch and a DB-backed job queue · Nix flakes for a one-command environment · a push-to-deploy platform for most products, bare metal with declarative OS config when the economics or control genuinely win. **One coherent stack rather than a menu**, because mixing halves of two stacks gets you the integration problems of both. Two honest caveats ship with it: Nix has a real learning curve, so *timebox the spike and keep a fallback* rather than yak-shaving; and a recommended stack is not a migration order for products you already run — **every new product names its choice explicitly, and inheriting an existing stack is a perfectly good reason stated once.**

**Your own declaration outranks it.** If `context/declared/coding_style.md` or `coding-practices.md` exists, **it wins** — the skill reads yours first and treats the recommendation as already answered. An operator who has decided has written it down, and re-litigating their stack every session is the opposite of useful. When the two disagree that is not drift: **a deviation is a decision**, recorded with its reason.
