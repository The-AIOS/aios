...
4. **Re-run the installers + verify the universal hooks are wired (not just the shell wrappers):**
   ...
   - **Event hooks in `~/.claude/settings.json` (verify — the wrapper installer does NOT touch these):** confirm BOTH universal hooks are present per SETUP §10 — `UserPromptSubmit` → `bash ~/aios/hooks/inject-datetime.sh` (real clock injected into every prompt) AND `statusLine.command` → `claude-identity.sh cache | context-monitor.py` (rate-limit cache + context display). `settings.json` is operator-personal + gitignored, so `/aios:update` cannot wire it for you — **your Claude session must read the file and ensure both entries are present AND point at the correct current commands: merge in what's missing, fix what's stale or wrong, and don't clobber unrelated settings (permissions, MCP registrations, a custom statusline, etc.).** **Verify both fired:** open a fresh session, ask *"what's today's date?"* (proves the datetime hook ran) and check the statusline shows context/quota (proves the statusLine hook ran). This closes the gap where only the spawn wrappers were re-enforced on update while the datetime + statusline hooks could silently be missing or stale.
