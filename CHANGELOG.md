...
> **What you can now do.** Trust that a worker you spawn arrives knowing who it works for — and does not spend the whole task finding out. **The map, always, whatever the task:** both context folders listed, both `_index.md` read, then **every heading in both folders** — globbed (`observed/*.md`, `declared/*.md`), **never a list of filenames**. Your two context folders are yours: the framework ships five declared files and nine observed ones, and you rename them, add your own, and write them in your own language. A floor that named files by name found **zero** entry titles in a vault whose files were renamed — and a floor at zero is indistinguishable from one that fired. A few hundred lines, and it is the floor and the index at once — a title tells a worker the lesson exists and when to open it, `antifragile.md` most of all, which the ritual already calls the file to scan before executing commands.
