...
**Action required:** one approval. Everything lands with your next `/aios:update`, and because the Opus 5.5 change is in the script that writes your shell profile, the update will **show you that change and ask before re-running it** — the only behavioural difference in it is that one model name. Open a new terminal afterwards for it to take effect. Two notes if a symptom sounds familiar: if your daily plan has felt an hour or two off and you mirror a work calendar into Google, that was the calendar fix, and it applies from your next `/aios:today`. If a previous `/aios:update` on Windows reported a large number of framework files as drift, that was this bug — nothing was lost, and the next run shows you the real list.
