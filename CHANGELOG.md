...
> **What you can now do.** Trust that a worker you spawn knows who it is working for. `CLAUDE.md` told spawned workers to run the full Session Start Ritual — and measured across one live vault's workers that did real work, **only 4 in 28 came close to it and 3 read nothing at all.** "Read everything" was not producing everything; it was producing an unmeasured fraction with no floor underneath and no signal when it hit zero.

**A cheap floor that always fires.** Every worker now reads the `###` entry titles of `observed/antifragile.md`, `preferences.md`, `patterns.md` and `growth.md` — titles only, a few hundred lines, roughly 4% of those files. Those four hold what you repeat, avoid, prefer, and have already been burned by, and **no task ever names them**. `antifragile.md` most of all: the ritual already calls it the file to scan before executing commands, and an entry title tells a worker both that a lesson exists and when to open it.

**What is skipped is named, and stays reachable.** The full set is ~150k tokens. A worker is told it is skipping most of that because the task does not need it — **not because it is off limits** — and that opening any of it mid-task is expected. If it catches itself guessing at how you would phrase something or whether you have decided something before, it stops guessing and reads.

**The rest is sized by the work, not by who is running.** One question about the worker's own output: *will this be read as your words, or act on your behalf?* Writing, deciding, advising, anything an audience attributes to you → **the full ritual**. Work checkable without knowing you — code, tests, file operations, data → the floor plus what the task touches. **Unsure is not a third answer: read everything.** Over-reading costs one session's tokens; under-reading costs your voice and fails silently.

**And it stays measured.** `/aios:housekeeping` gains a report-only bucket naming any spawned worker that did real work and loaded zero context files. The detector ships as `hooks/context-load-audit.py` rather than as instructions a command re-implements — the first version of that measurement matched only full paths, and workers routinely `cd` into the context folder first, so it reported workers at zero that had read the entire floor. It reads `tool_use` events only (a transcript mentions paths constantly; only a call is evidence), separates floor-depth reads from full ones, is unaffected by compaction (the transcript on disk is append-only), and **refuses to report findings unless a primary session validates it** — without a control, a worker reading nothing and a broken detector look identical.

**Action required:** none. Workers pick this up from `CLAUDE.md` immediately; the `spawn` wrapper prints the same rule at the top of every task file on both macOS/Linux and Windows, so re-run the wrapper installer if you want that half too — `/aios:update` does it for you when the installer changes.
