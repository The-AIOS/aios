...
**And it stays measured.** `/aios:housekeeping` gains a report-only bucket naming any spawned worker that did real work and loaded zero context files, with primary sessions as its control. A narrowing justified by a measurement has to keep being measured.
