...
> **What you can now do.** Trust that a worker you spawn arrives knowing who it works for — and does not spend the whole task finding out. Your two context folders are different in kind, and they are now read differently: **`declared/` is who you are** (voice, identity, how you decide — prose, ~20k words, and it does not index: a worker reads it or guesses at it). **`observed/` is how you and Claude work together** (preferences, patterns, lessons — ~100k words, written as entries, so it indexes beautifully).

**The map, always, whatever the task.** Both folders listed, both `_index.md` read, then the `###` **entry titles** of `observed/antifragile.md`, `preferences.md`, `patterns.md` and `growth.md`. A few hundred lines. That is the floor and the index at once: a title tells a worker the lesson exists and when to open it — `antifragile.md` most of all, which the ritual already calls the file to scan before executing commands.

**Then one question about the worker's own output:** *will this be read as your words, or act on your behalf?* **Yes → all of `declared/` plus `INTENT.md`** — and deliberately *not* all of `observed/` too. **No → the map is enough**, opening an entry when a title says it applies.

**What is skipped stays reachable.** A worker is told it is skipping `observed/` because titles index it better than prose does — **not because it is off limits** — and that reaching for any of it mid-task is expected. If it catches itself guessing at how you would phrase something or whether you have decided something before, it stops guessing and reads.

**Measured, three ways, on the same two-sentence task.** Under the old rule the worker made **zero tool calls** and produced a generic line. Told to read *everything*, it spent **38 calls and never delivered**. Under this split it delivered in **26** — in the operator's voice, using their own phrasing from their own files, applying a per-surface writing budget it found there, and checking `INTENT.md` before assuming it could publish.

**And it stays measured.** `/aios:housekeeping` gains a report-only bucket naming any spawned worker that did real work and loaded nothing. The detector ships as `hooks/context-load-audit.py` rather than as instructions a command re-implements — the first version matched only full paths and missed every worker that `cd`s into the context folder first, reporting workers at zero that had read the whole floor. It counts `tool_use` only, separates floor-depth from full reads, is unaffected by compaction (the transcript on disk is append-only), and **refuses to report unless a primary session validates it** — without a control, a worker reading nothing and a broken detector look identical.
