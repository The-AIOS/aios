...
**Why the *tail*, and why that is safe at any size.** Your two context folders are written differently, and that — not their size — is what decides how each is read. **`declared/` is restated**: identity you author, rewritten in place, no chronology, so there is no newest end to read. **`observed/` accumulates**: every file dated and append-ordered, so its newest end is exactly what your last sessions wrote. The tail is also **bounded** — at ten times the entries it reads the same five per file, while the headings still index all of them. *A bounded selector never goes stale; an unbounded volume does.* Recency is not relevance and the design does not pretend otherwise: every older entry is still one title away, and a worker is told that reaching for one mid-task is expected.

**Above the floor there are no tiers — only fit.** The operating definition the framework uses: **intelligence is the right information at the right time, not the largest pile you can afford.** So there is no level for a worker to reach and nothing to qualify for — the floor is a *starting position, not an allowance*. If five recent entries of `preferences.md` do not settle the question, it reads twenty, or the file. If one `declared/` file answers it, it reads that one rather than the folder. If the task names a venture, it opens that venture. **And later beats speculatively earlier**: reading your voice files at minute thirty, having just realised the deliverable goes out in your name, is better fit than reading them at minute one in case it might — so reaching mid-task is the *normal mode*, not a fallback. **Reading everything is the degenerate case, not the top of a ladder**: correct only when the whole context is cheaper than deciding what to skip, or when *the task is the context itself* (synthesis across your history, auditing or compacting the observed files, answering *as* you). Outside those, reaching for everything is not thoroughness — it is the absence of a judgment, and measured it costs 38 tool calls and produces nothing.
...
**No hand-picked token number survives anywhere in this.** The verdict that says *just read everything* used to flip at a token count someone chose. It now flips on a **ratio of the vault to its own floor**: if everything costs less than three times what every session already pays, deciding what to skip is the expensive part, so don't decide. Same answers on both known vault shapes — a lightly-written one is 2.3x and reads it all, a heavily-written one is 7.7x and narrows — with nothing picked by hand inside them.
