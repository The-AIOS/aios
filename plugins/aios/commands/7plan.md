...
- **Load-shape + leverage check (consult the `sustainable-cadence` and `leverage-points` skills):** is the week's plan *sustainable* for the operator — capacity is a design input, not an afterthought, and carried/slow/deferred work can be a legitimate quality-gate or rhythm, not drift — and do its priorities sit on the **highest-leverage** places (goals, rules, self-organization) rather than low-leverage busywork? Flag an over-full or low-leverage week under *This week's challenge*.
...
**Compass check:** {**Only when `growth.md` carries a `## Compass` with content under it** — otherwise omit this line entirely and say nothing about purpose (the offer belongs to `/close-day`, once, and only past the variation gate). The bet is the largest *destination* this vault tracks; the compass is what destinations are expressions of. So ask the one question at the one cadence where it is answerable: **is this bet an expression of the compass, and if not, which of the two is wrong?** A bet that does not express the compass is either a bet taken for someone else's reasons or a compass that has gone stale — and both are worth a sentence *before* the week's priorities are set, not after. Weekly is the right cadence for this: daily is too tight to see it and quarterly is too late to act on it. **Never resolve the mismatch by quietly editing the compass to fit the bet** — that inverts the whole thing. Name the tension and leave it with the operator.}
