...
> **Why the threshold moved (2026-07-27).** This guard used to read *">500 lines OR >50 entries"*, which **contradicted `compact.md`** after the compaction fix re-based the bound to tokens with **condense** as the primary lever. Two specs disagreeing about one file means one of them cries wolf: at 716 lines / 89 entries / ~36.9k tokens the old guard flagged on *both* counts while `compact.md` correctly reported *within bounds, skip*. A standing false alarm inside the bucket that exists to catch false alarms. **Entry count decouples from cost once entries are terse** — condensed entries are cheap, so counting them measures the wrong thing. Tokens are the cost; the entry cap is only a coarse backstop.
...
**The gap.** Every `antifragile.md` entry states a **system fix**. Buckets 9, 12, 15 and 19 check the file's *structure*, *volume*, *promotion* and *lifecycle* — **not one of them ever checks whether the stated fix actually shipped.** An entry claiming *"system fix: X"* where X was never applied is a check that cannot fail, sitting inside the file about checks that cannot fail (see meta-pattern / entry **#88**). This bucket closes that loop, and only that loop.
...
3. **Classify** — and note the asymmetry: only the first two are findings.
   ...
   - **UNCHECKABLE** (judgment-phrased, or the artifact lives on a machine/repo this vault can't see) → **not a finding.** Do not report it as a miss; a check that can't run is not a check that failed (that mistake *is* #88).
4. **Report** as: *"#N claims fix `{artifact}` — probe `{exact command}` returned {result}. Cause unknown: regressed / moved / aspirational? Operator confirms."*
5. **State COVERAGE — mandatory, every run, even when there are zero findings.** This bucket **samples**; it does not audit. A packet that reports only findings lets a silent cap read as *"everything checked out"*, which is the exact false-completeness shape this bucket exists to catch (**#88** — a clean result is meaningless until you know what it measured). So every run emits one line, unconditionally: *"Fix-verification: **{T}** entries in file · **{S}** sampled (recent 15 + artifact-naming) · **{P}** present · **{A}** absent · **{U}** uncheckable (judgment-phrased) · **{K}** skipped as out-of-sample."* The numbers must reconcile (`S = P + A + U`, `T = S + K`) — if they don't, the sampler is broken and that is itself the finding. **Never emit "no findings" without this line beside it.**

**Evidence discipline:** paste the **exact probe and its exact output**. *"#63 claims `hooks/git/pre-commit` installed; `test -e hooks/git/pre-commit` → not found"* beats *"some fixes look stale."* If you cannot state the probe, you do not have a finding.
