...
**Volume guard — use `/aios:compact`'s bound, and measure TOKENS, not entries.** Surface `antifragile.md` size in **both** tokens and entries: `wc -c ÷ 3.7 ≈ tokens` · `grep -c '^### [0-9]' = entries`. **Flag only against the bound `/aios:compact` Step 3.5 states — read it, and do NOT restate the numbers here.** Also honour any `USER.md` override of that bound, because an operator who re-aims it expects both commands to move together. If it's within bounds, say so explicitly and propose nothing.

> **Why a pointer and not a copy (2026-08-18, reported by an operator).** This line used to restate the numbers inline *while naming `compact.md` as the source of truth in the same sentence* — two homes for one bound, with a comment asserting they agreed. When the operator re-aimed the bound (which TIER 0's own measurement invites), `/aios:compact` honoured the override and this guard kept flagging the retired number: **a false 🔴 finding, an approved remediation scope, and a full worker session spent condensing a file that was inside its real bound** — with the disproof one grep away. A restated constant is a second implementation; the failure is not that it is wrong today but that nothing keeps it right tomorrow.

> **Why the threshold moved (2026-07-27).** This guard used to read *">500 lines OR >50 entries"*, which **contradicted `compact.md`** after the compaction fix re-based the bound to tokens with **condense** as the primary lever. Two specs disagreeing about one file means one of them cries wolf — and the old numbers are low enough that a file sitting *comfortably* inside `compact.md`'s bound trips the old guard on **both** criteria. A standing false alarm inside the bucket that exists to catch false alarms. **Entry count decouples from cost once entries are terse** — condensed entries are cheap, so counting them measures the wrong thing. Tokens are the cost; the entry cap is only a coarse backstop.

**If the bound IS exceeded — condense before you consider removing anything.** Hand the pass to `/aios:compact` Step 3.5 rather than improvising here, and respect its ordering: **(1) condense** verbose entries into what-broke / why / fix triplets — this is *the primary lever*, because once entries are terse, removal frees almost nothing and costs real wisdom; **(2) tombstone only entries the file itself explicitly marks** graduated / merged / superseded, keeping number + title + marker so every `#N` cross-reference and the meta-pattern index still resolve; **(3) surface anything unmarked for the operator to confirm — never act on inference.** Entry numbers are identity: never renumber. And per CLAUDE.md, *"never delete"* is operationally **"never delete without a snapshot"** — the snapshot is the precondition, not a courtesy.
...
**Why this bucket exists:** observed-context drift is invisible until you measure it (a real vault was found carrying several observed files months past their thresholds at once, with Reinforced session-insights sitting unrouted beside them — none of it visible from inside a normal session). Drift is silent by construction, so the bucket measures rather than asks. 11-49 days marked "Ready to route" but never executed). The forward `/close-day` mechanism (Tier A routing enforcement + Tier B observation pass) addresses the same gap proactively, but Bucket 19 is the periodic floor-check that ensures the forward mechanism is actually firing. Without it, an operator who skips `/close-day` for a week loses both layers silently. The reference-integrity layer was added after a 2026-06-15 catch: an operator's `vault-routine.md` still taught the dead `vault-commands:*` namespace (renamed to `aios:*` weeks earlier) on every session start — canonical's seed was clean, but the operator's evolved copy never caught the rename. A renamed command or namespace is silent rot in exactly the files read at startup; this layer makes it self-surface instead of waiting for an audit.
...
- **Cross-file key collisions:** the same key defined in two live roadmap files → propose a prefix rename in the younger file. **Read the collision's SHAPE first, because the common case is not two roadmaps.** A snapshot is a byte-for-byte copy, so every archived roadmap carries `type: roadmap` + `status: live` and answers the resolution test like the original — a vault that archives diligently shows *one* key colliding across *many* files. **The count is the tell: two files means two roadmaps; many files, all under `logs/`, means the grep was unscoped.** Re-scope (CLAUDE.md § Ship-time truth-flip, rule 1) and re-run before reporting anything. **Never propose renaming a key inside an archive** — an archive is a record, and a record may not be edited to settle a dispute it was never party to; the other wrong move is renaming the key in the *live* file to escape a conflict this bucket's own unscoped grep manufactured. Report a genuine collision only when every colliding file is a live roadmap the operator maintains. Generally: **a finding whose magnitude scales with the operator's good habits is a finding about the instrument.**
...
#### Bucket 26: Antifragile fix-verification (NEW — REPORT-ONLY, never edits)

**The gap.** Every `antifragile.md` entry states a **system fix**. Buckets 9, 12, 15 and 19 check the file's *structure*, *volume*, *promotion* and *lifecycle* — **not one of them ever checks whether the stated fix actually shipped.** An entry claiming *"system fix: X"* where X was never applied is a check that cannot fail — sitting inside the very file whose job is cataloguing checks that cannot fail. This bucket closes that loop, and only that loop.

> **HARD CONTRACT — this bucket is READ-ONLY on `antifragile.md`. It has no write path at all.** It never edits, annotates, tombstones, reorders, condenses or deletes an entry, and it never adds one. Its *entire* output is rows in the Phase-2 review packet. A finding here is **a question for the operator, never an action** — because "the fix isn't there" has three very different causes and this bucket cannot tell them apart: the fix genuinely regressed · the fix moved (renamed file, refactored path, graduated into `CLAUDE.md`) · or the entry's wording was always aspirational. Only the operator knows which.

**How to check (cheap, mechanical, bounded).** Do **not** audit all entries — sample. Take the **most recent 15 entries** plus any entry whose fix names a **path, filename, command, or hook** (those are the falsifiable ones; a fix phrased as a judgment — *"pause before asserting"* — is not mechanically checkable and is **out of scope, not a finding**). For each:

1. **Extract the claimed artifact** from the entry's `**Fix:**` / `**System fix:**` line — and extract **only an artifact the fix CLAIMS TO HAVE PRODUCED**. Three exclusions, each of which produced a false positive on the first real run (2026-07-27: **4 of 5 flags were extraction errors, 1 was a genuine regression**):
   - **A flag is NOT an artifact.** A fix can name a flag to *mandate* it or to *forbid* it — *"always pass `--foo`"* and *"never pass `--foo`"* both mention `--foo`. String presence proves neither, so a flag is **UNCHECKABLE** by construction. Do not probe it.
   - **A file the fix tells you to READ ELSEWHERE is not an artifact this fix creates.** A fix like *"inspect a plugin's manifest before installing it"* names a third-party file that the vault should never contain. Skip it — the fix produced a habit, not a file.
   - **When the fix's success condition is ABSENCE, presence is the failure — so never probe it as if present==good.** A fix that *reverted* an over-reach, deleted a stray tool, or retired a path is working precisely when the artifact it names is **gone**. If the fix line says removed / reverted / deleted / retired of the artifact it names, mark **UNCHECKABLE** and move on. Probing it inverts the verdict and reports success as failure.
2. **Verify it exists now — and resolve by BASENAME, following symlinks.** Entries name artifacts the way a human writes them mid-incident (`plugin.json`, `masters/roadmap.md`, `positioning.md`) — **not** as paths relative to any one root. So probe with `find -L {vault} {canonical} {~/.claude} -name "$(basename "$artifact")"`, plus `grep -c` for a string inside the file it names, `command -v` for a binary, `launchctl list` for a launchd job, or the settings key for a hook.
   > **Include `~/.claude/` in the roots, and exclude `antifragile.md` from the results.** Fixes routinely install machine-state there (a monitor script, a launchd plist, a settings key) — the one genuine finding on the first real run was a `~/.claude/monitors/` script that had vanished, and it was invisible until that root was added. Conversely, a match **inside `antifragile.md` itself** is the entry quoting its own artifact: self-referential, can never fail, and therefore worthless as evidence (it is what made two flag probes read PRESENT for the wrong reason).
   > ⚠️ **Two traps, both measured on a live vault 2026-07-27 — a naive probe scored a 100% false-positive rate here.** (1) A **bare filename joined to a root** misses every artifact that lives in a subdirectory: `test -e ~/aios/plugin.json` fails while `plugins/aios/.claude-plugin/plugin.json` exists. (2) **`find` does not follow symlinks without `-L`**, and `~/aios` is commonly a symlink to the vault — so an un-`-L`'d probe traverses *nothing* and every entry reads ABSENT. Every entry the first trial run flagged turned out to be a probe error rather than a regression. **If your first pass flags most of what it checks, suspect the probe, not the file** — that ratio is itself the signal.
3. **Classify** — and note the asymmetry: only the first two are findings.
   - **PRESENT** → nothing to report. Silent.
   - **ABSENT** → report as `unverified` with the exact probe that failed.
   - **UNCHECKABLE** (judgment-phrased, or the artifact lives on a machine/repo this vault can't see) → **not a finding.** Do not report it as a miss; a check that can't run is not a check that failed — conflating the two is the same unfalsifiable-check mistake this bucket exists to catch.
4. **Report** as: *"#N claims fix `{artifact}` — probe `{exact command}` returned {result}. Cause unknown: regressed / moved / aspirational? Operator confirms."* **The probe you print must be the command you actually ran, verbatim** — not a rendered template. On the first real run the report printed a `find -L … -name` string for artifacts the code had actually probed with `grep`, which made the finding unreproducible by the operator and hid *which* branch had failed. If you cannot paste the command as executed, you do not have a finding.
5. **State COVERAGE — mandatory, every run, even when there are zero findings.** This bucket **samples**; it does not audit. A packet that reports only findings lets a silent cap read as *"everything checked out"*, which is the exact false-completeness shape this bucket exists to catch (a clean result is meaningless until you know what it measured). So every run emits one line, unconditionally: *"Fix-verification: **{T}** entries in file · **{S}** sampled (recent 15 + artifact-naming) · **{P}** present · **{A}** absent · **{U}** uncheckable (judgment-phrased) · **{K}** skipped as out-of-sample."* The numbers must reconcile (`S = P + A + U`, `T = S + K`) — if they don't, the sampler is broken and that is itself the finding. **Never emit "no findings" without this line beside it.**

**Evidence discipline:** paste the **exact probe and its exact output**. *"entry #N claims `hooks/git/pre-commit` is installed; `find -L … -name pre-commit` → not found"* beats *"some fixes look stale."* If you cannot state the probe, you do not have a finding.
...
**How to check.** Scan the operator's own `.py` / `.sh` under `hooks/custom/`, `skills/custom/`, `plugins/custom/`, and any routine scripts, for a statement that **invokes** `claude -p` without `--tools` **and** `--setting-sources user,project`. An invocation that relies on `--allowedTools` counts as unguarded, with or without `--permission-mode`: the allowlist is additive to every *"allow always"* rule accumulated in `.claude/settings.local.json`, so on a vault used daily it inherits broad `Read` and `Bash`. Two disciplines make this quiet rather than noisy:
...
**Report as:** *"`{file}:{line}` invokes `claude -p` without `--tools` + `--setting-sources user,project`. If any text in that prompt comes from outside the vault (a fetched page, an email, a transcript), it is worth containing. Three of the four obvious fixes do not work — see `MODEL-ROUTING.md` § Verify an id before you trust it."*
...
**Then check the sandbox of the headless calls that run `Bash`** — those whose `--tools` includes `Bash`. Each should pass its own `--settings <file>`, and that file should carry:
- `sandbox.autoAllowBashIfSandboxed: false` — while it is `true` (the default) the sandbox approves every Bash command, so `--allowedTools` and `--permission-mode default` stop nothing.
- `sandbox.filesystem.denyRead` naming the secrets folder and every MCP token folder that exists on this machine (at least `~/.config/aios-secrets` and `~/.google_workspace_mcp`). A `permissions.deny` `Read(...)` rule does not count: it does not stop sandboxed `cat`.
