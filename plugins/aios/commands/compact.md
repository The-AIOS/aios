...
3.5. **Bound `antifragile.md` (size-gated — this check runs EVERY invocation, independent of the target month).**
   ...
   a. Measure `vault/00 - notes/context/observed/antifragile.md` — **tokens are the real cost; entry count is only a proxy for it.** If **≤ ~45k tokens** (≈1,000 lines) **AND ≤ ~120 entries**, skip — within bounds. Tokens: `wc -c` ÷ 3.7. Entries: the **structural probe** below — never a bare heading count.
