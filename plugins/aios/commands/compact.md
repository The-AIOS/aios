...
3.5. **Bound `antifragile.md` (size-gated — this check runs EVERY invocation, independent of the target month).**
   ...
   a. Measure `vault/00 - notes/context/observed/antifragile.md` — **and measure the quantity that actually reaches a session, which is no longer the file.** `antifragile.md` is a **rule library**: the context floor emits its meta-pattern index and every entry title, and **none of its bodies**. So the per-session cost is **index + titles**, scaling with entry *count* and *title length* — not with file size and not with how verbose the bodies are. Verified on a live vault with a control: every one of the file's entry titles appeared in the floor and **zero** of its newest bodies did, while the newest bodies of the accumulating observed files all did.
      - **Session bound (the one that matters): ≤ ~120 entries**, and keep titles short — that is what the index carries. Entries: the **structural probe** below, never a bare heading count.
      - **File bound: ≤ ~250k bytes**, a disk-and-diff guard rather than a context one. A file over it is unwieldy to edit, review and snapshot; it is not, by itself, costing sessions anything.
      - Within both → skip. **Do not simply raise a number when one trips** — ask first which quantity it measures, because that is the defect this gate already had once: it bounded ~45k tokens of file on the premise that the file is "read every session", and that premise expired when the floor started emitting rule libraries as index-plus-titles. Measured then: ~180 KB on disk, ~7% of it reaching a session.

      ```bash
      # entry probe — count, highest and duplicate numbers in one pass; entries live OUTSIDE the meta-pattern index
      A="${A:-vault/00 - notes/context/observed/antifragile.md}"
      body(){ awk '/^## Meta-patterns/{s=1;next} /^## /{s=0} !s' "$A"; }
      n=$(body | grep -c '^### [0-9]\{1,\}\. ')
      hi=$(body | grep -o '^### [0-9]\{1,\}\.' | tr -dc '0-9\n' | sort -n | tail -1)
      dup=$(body | grep -o '^### [0-9]\{1,\}\.' | sort | uniq -d | wc -l | tr -d ' ')
      echo "entries=$n highest=$hi duplicated=$dup"
      ```

      **The invariant: `duplicated == 0` — and `entries ≤ highest`.** A **gap** (`entries < highest`) is legal and expected: removal leaves one, and the rules below call that correct because entry numbers are identity. A **duplicate** is not — any `duplicated > 0` means structural damage, most likely a stale copy of the file glued in by a bad edit (the silent-duplication class) — the pass MUST stop and diff the copies before compacting, because tombstoning against a doubled file can destroy the one copy carrying the newest content. _(Why the old quick-read is retired: a bare `grep -c '^### [0-9]'` counts a numbered meta-pattern index and date-prefixed headings as entries — a real vault measured **207** against **107 distinct** while carrying a complete duplicate of itself, and the inflated count **concealed** exactly the damage a measurement exists to expose.)_

      **Also measure the meta-pattern index separately and REPORT it.** (`## Meta-patterns` → the next `## ` heading; same `wc -c` ÷ 3.7 — the boundary is the section, never a heading shape, because the index may be numbered or lettered, and it is the same boundary the context floor uses.) **For a rule library the floor emits that whole section plus every entry title, and no entry bodies** — so the index is paid on every session start, which makes its size worth surfacing even though the bounds above gate on entries and bytes. It tells you **which lever to reach for** — index large relative to the file → TIER 0 (relocate); file large with a small index → TIER 1/2/3 (condense). Reported, not enforced. _(Raised 2026-08-18 by an operator who measured 64.7k whole-file against a 4.86k index: the whole-file gate fired, the pass reclaimed ~11% because those entries were already condensed, and the index — the part a vault actually pays on every read — had no bound anywhere.)_
   b. If over: **snapshot first** — `~/aios/hooks/aios-snapshot "vault/00 - notes/context/observed/antifragile.md"`, never a hand-rolled `cp` (CLAUDE.md § Session End: the destination is keyed by day and filename with no writer in it, so two passes archiving the same file the same day collide and `cp` reports success). The snapshot IS the archive — no separate `-archive.md` file; `/trace` reads snapshots + git history.
   c. Then compact the LIVE file in **three tiers** — never silent-delete on inference:
      - **TIER 0 — RELOCATE, and try this BEFORE condensing (added 2026-08-11):** a long argument living inside the meta-pattern index is paid in full every time the index is read, while the same prose in its own numbered entry is paid only when something points at it. **This was once hedged and the hedge is now obsolete, in the lever's favour.** It used to read *"under canonical's § Session Start — which reads every observed file in full — the index is not privileged"*; canonical no longer reads observed files in full, and for a rule library the floor emits **the index and the titles and nothing else**. So relocating prose out of the index is not merely shrinking the part most likely to be re-read — it is shrinking **the only part that is read at all**, which makes this an unconditional per-session saving. The lever is right either way; the justification had assumed a reading model canonical does not ship. Moving one such block out and leaving a two-line pointer measured **~1,570 tokens reclaimed in a single edit** — against **12.6% for condensing 24 of 50 entries**, because mature entries are dense rather than padded. Relocation is the lever that pays; condensation is largely spent after its first pass. ⚠️ **If the relocated block contains a live open decision, the pointer must carry that decision verbatim** — otherwise you have moved a decision into a file nobody opens, which is worse than the tokens you saved. _(Measured and reported by Luigi Matrone / ALI, 2026-08-08, from a real compaction pass.)_
      ...
      - **TIER 2 — CONDENSE (an editorial lever, NOT a session-cost lever):** **it frees roughly zero per-session tokens**, because bodies are not what a session loads (see 3.5a). Measured: condensing the four largest entries of a live file freed ~2% of the file and nothing a session pays for. Condense for readability and for the disk bound — never as the way to reduce what the file costs a session; reach for TIER 0 and the entry count for that. rewrite verbose entries into tight **what-broke / why / fix** triplets (~6-9 lines). Preserve verbatim: every entry number, all exact commands, flags, paths, hashes, and cross-references (`#N`, `[[wikilinks]]`, meta-pattern letters). Once entries are already terse, removal frees almost nothing either. Condensing is an editorial pass, not a deletion, so the wisdom survives at a fraction of the cost.
...
- **Antifragile (Step 3.5) is size-gated, not month-gated** — it runs every invocation and operates on the LIVE `antifragile.md` (the only compaction touching a current file, not a dated log). Always snapshot before compacting it; **"never delete" → "never delete without a snapshot."** Keep the meta-pattern index + active recent lessons. **Tombstone only entries the file explicitly marks** (graduated/merged/superseded), **condense verbose entries** for readability and the disk bound (not for session cost — bodies are not loaded), and **surface unmarked stale candidates for the operator to confirm** — never auto-delete on inference (a wrong cut loses real wisdom, and Claude can't see what's load-bearing on a teammate's vault).
