...
3.5. **Bound `antifragile.md` (size-gated — this check runs EVERY invocation, independent of the target month).**
   ...
   a. Measure `vault/00 - notes/context/observed/antifragile.md` — **and measure the quantity that actually reaches a session, which is no longer the file.** `antifragile.md` is a **rule library**: the context floor emits its meta-pattern index and every entry title, and **none of its bodies**. So the per-session cost is **index + titles**, scaling with entry *count* and *title length* — not with file size and not with how verbose the bodies are. Verified on a live vault with a control: every one of the file's entry titles appeared in the floor and **zero** of its newest bodies did, while the newest bodies of the accumulating observed files all did.
      ...
      **The invariant: `entries == highest` AND `duplicated == 0` — a count that does not match its own highest number is not a measurement.** Any mismatch means structural damage, most likely a stale copy of the file glued in by a bad edit (the silent-duplication class) — the pass MUST stop and diff the copies before compacting, because tombstoning against a doubled file can destroy the one copy carrying the newest content. _(Why the old quick-read is retired: a bare `grep -c '^### [0-9]'` counts a numbered meta-pattern index and date-prefixed headings as entries — a real vault measured **207** against **107 distinct** while carrying a complete duplicate of itself, and the inflated count **concealed** exactly the damage a measurement exists to expose.)_

      **Also measure the meta-pattern index separately and REPORT it — the whole-file numbers above remain the gate.** (`## Meta-patterns` → the next `## ` heading; same `wc -c` ÷ 3.7 — the boundary is the section, never a heading shape, because the index may be numbered or lettered.) The gate stays whole-file because **canonical's § Session Start reads every observed file in full**, so as canonical ships, the whole file *is* the per-read cost and an index bound would gate on a reading model canonical does not implement. But the index number is worth surfacing: it tells you **which lever to reach for** — index large relative to the file → TIER 0 (relocate); file large with a small index → TIER 1/2/3 (condense). Reported, not enforced. _(Raised 2026-08-18 by an operator who measured 64.7k whole-file against a 4.86k index: the whole-file gate fired, the pass reclaimed ~11% because those entries were already condensed, and the index — the part their vault actually pays on every read — had no bound anywhere. Their vault tiers session-start reading, which inverts the ratio; if tiered reading ever comes to core, the gate moves with it.)_
