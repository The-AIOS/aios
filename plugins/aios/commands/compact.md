...
3.5. **Bound `antifragile.md` (size-gated — this check runs EVERY invocation, independent of the target month).**
   ...
   c. Then compact the LIVE file in **two safety tiers** — auto-remove ONLY what the file itself declares redundant; never silent-delete on inference:
      - **AUTO-remove (safe — mechanical):** entries **explicitly marked** as graduated/merged/superseded — i.e. carrying a `→ Graduated to CLAUDE.md`, `→ Merged into #N`, or `superseded by #N` marker. The file itself declares these redundant (the lesson now lives in CLAUDE.md or another entry), so removing them is safe.
      - **SURFACE for confirmation (judgment — never auto):** entries that *look* stale (resolved/shipped, old, low-recurrence) but carry **no explicit marker**. List them with the reason and **ask the operator to confirm** before removing each. NEVER auto-delete an unmarked entry on inference — a wrong cut loses real wisdom, and on a teammate's vault Claude can't see what's still load-bearing for them.
      - **Keep** the meta-pattern index, every **active/recent** lesson (last ~90 days or still-recurring), and any entry the operator didn't confirm.
   d. Report: entries auto-removed (with their marker), entries surfaced-for-confirmation (+ operator's decision), entries kept, before/after size. Every removal is reversible — the 3.5b snapshot + git preserve the full pre-compaction file.
