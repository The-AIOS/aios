...
1. **Run executor + read vault** — fire these in **one parallel batch**:
   ...
   - Read the daily note to close: **if invoked with a date** (*"close {date}"* — `/today`'s precondition guard does this), close exactly that note. Otherwise list files in `01 - calendar/{YYYY-MM}/`, pick the **most recent `YYYY-MM-DD.md` dated on or before today** (exclude weekly plans like `W{N}-plan.md`, and the future-dated skeletons `/7plan` creates for the week ahead). If it's after midnight and no note exists for today, the most recent note is yesterday's — close that one. If it already has a `## Close of Day` section, update it (merge new info, don't duplicate). **Always tell the user which date you're closing:** "Closing {date}."
...
**Existence gate:** `Grep` for `type: roadmap` frontmatter across `00 - notes/` **excluding `00 - notes/logs/`**; keep files whose `status:` isn't `archived`. **The exclusion is load-bearing** — a snapshot is a byte-for-byte copy, so every archived roadmap carries `type: roadmap` + `status: live` and passes this gate exactly like the real file (CLAUDE.md § Ship-time truth-flip, rule 1). **None → skip this entire section silently** — most vaults track everything in project notes, and that's the design, not a gap. (The roadmap layer is opt-in: `templates/aios/vault-grammar.md` § 7 defines when a session should *recommend* its birth; this section is what makes the layer surface daily once it exists.)
...
**Where to look — the CANONICAL harvest dir.** **Every** project/worker/agent `/close-session` (Mode B) writes its report to **`~/aios/.claude/session-report-{YYYY-MM-DD}-{project}-{session}.md`** — one dir for all of them, so this single glob catches every Mode B report regardless of which repo it ran in (no dependency on the repo being a registered Dev project — this is the seam-2 fix). Run `ls ~/aios/.claude/session-report-{YYYY-MM-DD}-*.md` — `{YYYY-MM-DD}` is **the date being closed**, not the clock's date (they differ after midnight, and a report written for yesterday must be harvested into yesterday's note) — (the `-*` globs every project+session; a Close-all broadcast lands one file per session), then `Read` each. **After harvesting, delete the consumed report files** (they're transient bridges; `rm` them so the next day starts clean).
...
**Then the other direction — the silent-badge check.** The pass above only catches the *loud* failure (a suggestion whose task is already done). Count the `- 🤖` bullets in the section against how many carry a **bindable** target — a `[[wikilink]]` or a **backticked** `` `/command` `` (the slash and the backticks are both required, so `` `spawn ingest` `` and a bare `/aios:ingest` both bind nothing). Any bullet with no bindable target is **invisible to both surfaces**: the badge reads low or zero while the note lists the tasks, and nothing else in the system reports it. Say it plainly — *"{N} of {M} bullets in the agents mirror carry no dispatch target, so the surfaces cannot see them"* — then either route it (`` `/aios:ingest` `` for an ingest) or move it out of the mirror into an ordinary `- [ ]` task. Also check the section's `**N tasks…**` header against the bindable count; it is prose the parsers ignore, so it drifts silently.

This is the dumb, reliable half: `/today`'s writer rule prevents the drift and this notices when it happened anyway. It must run regardless of whether anyone suspected a problem — an under-count produces no error, no phantom and no missing UI, so there is nothing to prompt a check.
...
2. Read all project notes (batched in groups of 10) — **payload discipline by section, not by line count:** read the frontmatter, then `Grep` for `## Current State` and `## To-Dos` and read each section **in full** from its offset to the next `## ` heading. Skip architecture, session notes, decisions log, links. A fixed `limit:` truncates any note whose to-dos start below it, and the snapshot written from that read still carries today's date — so it looks complete and is not.
...
- `session-insights.md` — scan and garden the observation buffer (reinforce, add, route, clean up). **Garden the two classes differently** (CLAUDE.md § Observed Context Rules): `behavioural` entries follow the Emerging → Reinforced → Routed lifecycle below; **entries marked `class: method` are not waiting for anything** — a second sighting would mean the fix never landed, so sweep every method entry for its exit at each close (**route** a new class · **fold** into an existing entry it sharpens · **drop** when an existing rule already covered it). Most fold or drop. Caps ≤10 Emerging, ≤5 Reinforced — **an over-cap Emerging section is almost always undisposed `method` entries, not stale behavioural ones**, so sweep those first rather than reaching for the cap.
  ...
  **Measure it rather than reading it.** The buffer costs ~13-22k tokens to read in full, which is why its cap and clock drifted while they were enforced by impression. Get the state for a couple of hundred instead:
  ...
  ```bash
  UV_CACHE_DIR="${TMPDIR:-/tmp}/uv-cache" uv run ~/aios/hooks/buffer-status.py
  ```
...
1. **Read feed-in sources** (durably — re-read the surfaces; do NOT rely on carrying candidates in working memory from the harvest steps above):
   - **PRIMARY (all three files) — the candidates the capture→harvest flow deposited THIS close-day:** every session report's `Observed (Tier B candidates)` (§ Dev session reports + § Agent session reports) **+** every daily-note `## Session —` block's `Observed (Tier B candidates)`. This is the whole point of the pipeline — the sessions tagged these for you; read them **explicitly here** so none is silently dropped when a long close-day harvested many. The file-specific sources below are **supplementary** — they catch growth/identity/relationship signal that landed in session-insights / antifragile / project-notes *without* a candidate tag.
   - `growth.md` ← recent `antifragile.md` entries (last 7 days — note: growth content sometimes lands in antifragile by mistake) + Reinforced entries with self-shape (about the operator changing, not about work mechanics) — **both those still in `session-insights.md` and those the Tier A pass above routed out of it today**: that pass runs first and excises what it routes, so read those where they landed, via today's markers in the buffer beginning `<!-- ROUTED {date} (Tier A)` — follow each marker's `[[target]]` link to read the entry where it landed — + close-day `### Observed` sections from the last 5 daily notes
...
4b. **Aggregate re-derivation (ecosystem, and partly profile) — periodic + holistic, NOT atomic.** Steps 1–4 are *atomic-append*: they capture new points, one event at a time. But `ecosystem.md` is a **derived aggregate** — a relationship map — and a map cannot be kept current by point-appends alone (a redraw re-synthesizes nodes that already exist elsewhere, so the atomic uniqueness test rejects it, and shifts filed to project notes / `USER.md` / `profile.md` never trigger a redraw at all). So on top of the atomic pass, **`ecosystem.md`** gets a **full-map re-derivation** whenever its own **`rederived:`** frontmatter date is older than the aggregate threshold `hooks/observed-staleness.py` applies (`AGGREGATE_DAYS`), or absent (`profile.md` is only *partly* an aggregate and has no full redraw defined, so this clock is `ecosystem.md`'s alone; `profile.md` keeps its atomic path and the staleness alarm) — **not** `updated:`. Every atomic append in step 3 sets `updated:` to today, so a map that receives one observation every few weeks never looks stale by `updated:` and is never redrawn: the clock that decides the redraw has to be one that only the redraw moves. After a redraw, set **both** `rederived:` and `updated:` to today. After an *attempt* that finds nothing to redraw — a seed file with no relationships yet, where step 4's substance bar says don't write — set `rederived:` alone, so a new vault is not redrawn at every close. The re-derivation itself: re-read the entire relationship graph across *every* widened feed-in surface (project-note changes + `USER.md` tables + `profile.md` threads + `business.md`) and **redraw the map wholesale**. Log the mode in the digest — *atomic-append* (event-triggered, steps 1–4) vs *aggregate-rederive* (periodic, holistic). The natural home for a *scheduled* monthly re-derivation is `/aios:compact` or an `/aios:housekeeping` bucket; until that lands, this `rederived:` check at close-day is what guarantees the redraw happens rather than waiting on a cadence command.
...
1. Archive it with the helper — **never a hand-rolled `cp`**:
   ...
   - If you already edited the file, archive the committed version instead: `git show HEAD:"vault/00 - notes/context/observed/{file}" > "$TMPDIR/{file}"`, then run the helper on `"$TMPDIR/{file}"` (it keys the archive by the file's name, which is unchanged)
