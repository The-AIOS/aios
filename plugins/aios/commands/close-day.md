...
1. **Read feed-in sources** (durably — re-read the surfaces; do NOT rely on carrying candidates in working memory from the harvest steps above):
   ...
   - `growth.md` ← recent `antifragile.md` entries (last 7 days — note: growth content sometimes lands in antifragile by mistake) + Reinforced entries with self-shape (about the operator changing, not about work mechanics) — **both those still in `session-insights.md` and those the Tier A pass above routed out of it today**: that pass runs first and excises what it routes, so read those where they landed, via today's `<!-- ROUTED {date} (Tier A): {title} → [[{target}]] -->` markers in the buffer + close-day `### Observed` sections from the last 5 daily notes
