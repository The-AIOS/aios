...
- `Bash(u=$(git -C ~/aios remote get-url origin 2>/dev/null); case "$u" in "") echo "backup: none";; *[Tt]he-[Aa][Ii][Oo][Ss]/aios*) echo "backup: framework-origin";; *) echo "backup: ok";; esac)` — **vault backup check.** Setup lets an operator finish without GitHub (a failed or absent login must never block the install) and leaves the vault with no remote. This is how that stays visible: it reads the remote itself rather than a marker, so it cannot go stale and clears the day a private remote exists. `backup: ok` → say nothing. `none` or `framework-origin` → render per § Backup warning rendering below.
