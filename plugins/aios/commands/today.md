...
**The executor returns TWO calendar blocks, and the second one is load-bearing.** `## Google Calendar — Primary (…)` is today. `## Google Calendar — Next 14 days (AUTHORITATIVE — reconcile vault dates against this)` is the lookahead, grouped under one `**YYYY-MM-DD (Weekday)**` header per day. **When it is present, read it every run and reconcile the vault against it** — it is not optional colour for the Horizon line. (No Google calendar configured → there is no lookahead block; skip this section silently rather than treating the absence as an error. The lookahead reads the **primary** account only: events that live solely on a second account or an unmirrored calendar are absent from it, not missing from the operator's life.)

- **Any date the vault states about a future meeting is a COPY.** Project notes, `_index.md` snapshots, weekly plans and yesterday's Horizon all restate dates that someone wrote once. The invite is the authoritative source; they are not. When the lookahead **has** the event and the dates disagree, **the calendar wins and the vault gets corrected in the same session** — a date that has rotted reads exactly like one that has not, so nothing else will catch it. When the lookahead has **no** matching event, never change the vault: say so, because the event may live on a calendar the lookahead cannot see.
...
- `Bash(bash ~/aios/hooks/freshness-probe.sh --aios-update)` — **infrastructure freshness check.** Reads `.aios-update`, asks the framework upstream for current HEAD via `git ls-remote` (SSH, falling back to public HTTPS so a fresh HTTPS clone with no SSH keys still resolves instead of showing a false "unreachable"), compares hashes. Returns one of: `synced` (no action), `BEHIND ...` (upstream has new commits), `access-denied (why)` (the server answered and **refused the credential** — a removed key or lost repo access, not an outage), `unreachable (why)` (no answer — genuinely offline), `no-config` (no `.aios-update` tracker found — single-vault user). Surface the result per § Aios-update freshness rendering below.
  > **Why this is a script that keeps git's error output.** The inline version ran `git ls-remote … 2>/dev/null`, so a refused key and a dead network printed the same empty result, and the rendering below calls `unreachable` harmless — an operator whose machine key had been removed from their GitHub account was told *"offline — fine"* every morning for weeks. The script classifies the error text (`Permission denied` · `Repository not found` · `could not read Username` · `Authentication failed` → `access-denied`; anything else → `unreachable`). `tests/freshness-probe.test.sh` holds a control that reproduces the old behaviour.
- `Bash(cal=~/aios/vault/01\ -\ calendar; t=$(date +%Y-%m-%d); if [ ! -d "$cal" ]; then echo "close-day-precondition: no-calendar-folder ($cal)"; elif ! lst=$(find "$cal" -maxdepth 2 -type f); then echo "close-day-precondition: calendar-unreadable ($cal)"; else prev=$(printf '%s\n' "$lst" | grep -E '/[0-9]{4}-[0-9]{2}-[0-9]{2}\.md$' | awk -F/ -v t="$t" '{d=substr($NF,1,10)} d<t{print d" "$0}' | sort -r | head -1 | cut -d' ' -f2-); if [ -z "$prev" ]; then echo "close-day-precondition: no-prior-note"; elif grep -q "^## Close of Day" "$prev" 2>/dev/null; then echo "close-day-precondition: closed ($(basename "$prev" .md))"; else echo "close-day-precondition: MISSING ($(basename "$prev" .md)) — must run /close-day first"; fi; fi)` — **close-day precondition check.** Detects whether the most recent daily note BEFORE today has the `## Close of Day` heading. Returns one of: `closed (date)` (previous note closed — proceed normally), `MISSING (date)` (skipped close-day — guard fires per § Close-of-day precondition rendering below, BEFORE Message 1b runs), `no-prior-note` (first run / fresh vault — proceed normally), `no-calendar-folder` (the vault path is wrong — say so in one line; it is not a first run), `calendar-unreadable` (`find` could not walk some folder — a permission problem; say so and do NOT proceed as if there were no prior note, because the unclosed note may be exactly the one it could not read). "Before today" is a date comparison, not "anything but today": a note dated in the future (a pre-created plan) can never be taken for yesterday's.
- `Bash([ -f ~/aios/vault/.pending-quota-autopilot-capture ] && echo "pending-setup: quota-autopilot-capture" || echo "pending-setup: none")` — **deferred-setup check.** **The Day-7 check-in leaves this marker** when the operator opts into multi-account autopilot but the capture dance (login/logout/Keychain) is deferred, because running it inside a live session would break that session. *(It used to say "SETUP §11 leaves this marker" — that step asked the multi-account question during setup and no longer exists: the question was unanswerable on day one and moved to Day 7, conditional on the caps having actually bitten. This reader outlived its writer for one commit; the writer is now the Day-7 flow.)* If marker present → surface as the FIRST task in today's Rhythm (see § Pending-setup rendering below). Operator can complete it today or carry forward; the daily plan tolerates either choice.
- `Bash(g(){ git -C "$HOME/aios" "$@"; }; if [ -z "$(g remote 2>/dev/null)" ]; then echo "backup: none"; else b=$(g symbolic-ref --short -q HEAD); r=$(g config "branch.$b.pushRemote" || g config remote.pushDefault || g config "branch.$b.remote" || { [ "$(g remote | wc -l)" -eq 1 ] && g remote; } || echo origin); u=$(g remote get-url --push --all "$r" 2>/dev/null); if [ -z "$u" ]; then echo "backup: none (push remote '$r' has no URL)"; elif printf '%s' "$u" | grep -qiE '[:/]the-aios/aios(\.git)?/?$'; then echo "backup: framework-origin ($r)"; else echo "backup: ok"; fi; fi)` — **vault backup check.** Setup lets an operator finish without GitHub (a failed or absent login must never block the install) and leaves the vault with no remote. This is how that stays visible: it reads the remote itself rather than a marker, so it cannot go stale and clears the day a private remote exists. **Any remote counts, whatever it is named** — reading only `origin` would warn every morning, wrongly, on a vault whose private remote has another name. And the framework check looks only at the remote `git push` would actually use, matched exactly: a vault that keeps the public repo as a second, fetch-only remote (`upstream`, say) is fine, and `The-AIOS/aios-app` is not the framework. `backup: ok` → say nothing. `none` or `framework-origin` → render per § Backup warning rendering below. Every push URL of that remote is checked (`--all`): `git push` sends to all of them, so a framework URL listed second is still a publish. The remote is resolved the way `git push` resolves it: an explicit push remote, else a lone remote whatever its name, else `origin`. A push remote that resolves to no URL (several remotes, none configured for push, none named `origin`) is `none`, never `ok` — "no URL" is not "not the framework".
- `Bash(m=~/aios/vault/.pending-day7-checkin; if [ -f "$m" ]; then d=$(grep ^due= "$m" | cut -d= -f2); t=$(date +%F); if [ -n "$d" ] && [ "$(echo "$t" | tr -d -)" -ge "$(echo "$d" | tr -d -)" ]; then echo "pending-setup: day7-checkin due=$d"; else echo "pending-setup: day7-checkin pending=$d"; fi; else echo "day7: none"; fi)` — **Week-1 check-in check.** `/aios:cold-start-interview` Step 10 tells the operator *"I'm scheduling a Day-7 check-in"*, and this is what makes that true: the interview writes the marker with a `due=` date, and this probe surfaces it on or after that date. **The promise came before the mechanism** — for one commit the interview said this out loud and nothing wrote or read anything, which is the same defect as re-timing a step to a destination that does not exist. When `due` has arrived, render it per § Week-1 check-in rendering; when it has not, stay silent (a countdown is noise).
  > **The comparison is an integer compare on the date with its dashes stripped, deliberately** — the first version was `[ "$t" \>= "$d" ]`, and `test` has no `>=` operator: it printed *"binary operator expected"*, returned non-zero, and fell to the else branch, so the check-in would have stayed **silent forever** while looking like it was merely waiting. Verified across the boundary (day before / due date / day after) rather than assumed. Delete the marker once the check-in runs: `Bash(rm -f ~/aios/vault/.pending-day7-checkin)`.
- `Bash(bash ~/aios/hooks/freshness-probe.sh --companies)` — **per-company freshness check.** Iterates over every mounted company in `vault/00 - notes/context/ventures/*/`, reads its `.{company}-sync` tracker, asks the remote for HEAD via `git ls-remote`, compares. Returns one line per mounted company: `company-{name}: synced` / `BEHIND ...` / `access-denied (why)` / `unreachable (why)` / (skipped if no tracker). Surface per § Company freshness rendering below — parallels the aios-update check but covers venture-context drift, not framework drift. A company context is usually a **private** repo, so `access-denied` is the likelier failure here than for the framework.
  > **The glob this replaced errored on a day-zero vault.** `for venture in .../ventures/*/` has no match before the operator mounts anything, and **zsh treats an unmatched glob as a fatal error** (`no matches found`, rc=1) rather than passing the literal through the way bash and sh do — so the very first `/today` a new operator ever runs threw a red error at them, in the step this whole flow builds toward. Measured 2026-08-27 against a day-zero tree: `bash` rc=0, `sh` rc=0, `zsh` rc=1. `find` cannot fail this way and is what the previous-daily-note probe already uses. `hooks/freshness-probe.sh` keeps it.
- `Bash(uv run ~/aios/hooks/bus-dead-letters.py)` — **bus dead-letter check.** Prints `bus-dead-letters: none`, or one `bus-dead-letter:` line per item with `to` / `action` / `reason` / `at`, plus one `bus-unclaimed:` line per request **nobody ever claimed** (a plain `*.json` older than 5 min — pickup renames to `.holding`, so a legitimate wait is never flagged); an unreadable file is still reported by name *with the reason*, and it always exits 0 so "nothing found" is never read as a failed command. **Runs through `uv`** — the same runner Message 1a already uses for `pipeline-executor.py`, so it is not a new dependency. Do **not** write `python3` here: on Windows that name does not exist — the Microsoft Store execution alias intercepts it and prints an installer notice — so the check emits no `bus-dead-letter:` or `bus-unclaimed:` lines and reads exactly like a clean result, a silent false negative inside the one check built to end silent failures. **The scan is in a script on purpose — do NOT inline it back into the shell.** Both shell idioms for globbing a directory break, in opposite directions: `for x in $(ls dir/*.undelivered)` needs word-splitting that **bash does and zsh does not** (zsh joins every path into one string, so N files read as 1 bogus filename and the error gets swallowed — a *silent false negative inside the feature built to end silent failure*), while `for x in dir/*.undelivered` needs an unmatched glob to be harmless, which **bash tolerates and zsh treats as a hard error** — and the empty case is the common one. A `send`/`spawn` request the bus could not deliver is retired to `~/.aios/spawn-inbox/<name>.json.undelivered`, carrying `_undelivered` (`reason`, `at`, `surface`). **Nothing reads that directory.** Retiring a request stops it *blocking another surface*; it does **not** deliver it — and the bus README's *"nothing rots"* promise is about **contention**, not about messages, which is exactly the ambiguity that lets a dead letter sit unnoticed. So a verified-FAILED send is otherwise indistinguishable from a success to whoever sent it. Surface per § Bus dead-letter rendering below.
...
**Aios-update freshness rendering (handled in Message 1a, BEFORE Message 1b vault reads):**
...
- **`access-denied`** → **not an outage — a credential problem, and it does not fix itself.** The server answered and refused this machine's key or account, so `/aios:update` cannot reach the framework either. Surface it as a ⚙️ task in the Parking lot, not a soft mention: name the repo, the refusal in the parentheses, and the fix (add this machine's SSH key back to the account, or restore the account's access to the repo). Proceed to Message 1b.
- **`unreachable`** → soft mention in Energy note or as a one-liner near the bottom: *"aios-update check unreachable today (offline — fine for now)."* Don't escalate. Proceed to Message 1b. **Only this state may be called offline.**
- **`no-config`** → silent (single-vault user, nothing to sync). Proceed to Message 1b.

**Company freshness rendering (handled in synthesis, Message 2 — does NOT block Message 1b like aios-update does):**
...
`access-denied` companies get a ⚙️ task in the Parking lot, one per company: *"`{name}` context can't sync — {why}. Add this machine's key back, or restore the account's access to the repo."* Never fold it into the offline line: a company context that cannot be pulled goes stale silently, which is the failure this check exists to catch. `unreachable` companies get a quieter single-line mention in Observed section (not a callout). `synced` companies stay silent. If 0 companies are mounted → entire section omitted.
...
- **`MISSING`** → **guard fires.** Skipping /close-day breaks the cascade chain (daily note → project notes → `_index.md` snapshots → observed context). Without the cascade, today's plan reads stale data. Surface to user and run /close-day on the previous note BEFORE proceeding with /today's vault reads:
  ...
  Then invoke `Skill(aios:close-day)` **with the date the guard detected** (*"close {date}"*) and **wait for it to complete.** Name the date explicitly: left to itself, /close-day picks the most recent daily note, and when today's note already exists (created by `/7plan` or an earlier `/today`) that is today's — not the unclosed one this guard found. After /close-day finishes (the previous note now has the heading + the cascade landed), proceed with Message 1b. **Do not skip** — the cascade is critical for /today's accuracy. /today's value compounds on cascade integrity; without it, AIOS stops compounding (carries don't reach project-notes, snapshots stale, observed context never receives the day's insights, self-update integrity breaks).

**Bus dead-letter rendering (handled in Message 1a, alongside the aios-update check):**

- **`bus-dead-letters: none`** → silent.
- **One or more `bus-dead-letter:` lines** → surface a callout at the top of the plan, one line per item: `> 📮 **Undelivered handoff** — a request addressed to `{name}` ({action}) was retired without being delivered: *{reason}* ({at}). The work it asked for **did not happen and nobody was told.** Re-drop it, or hand the task over directly.` Then ask once whether to re-drop the request or discard it — do **not** auto-re-drop, because a stale request can be days old and its instructions may no longer apply.

- **One or more `bus-unclaimed:` lines** → a request **nobody ever picked up**, which is a different failure from a dead letter and needs a different answer. Surface one line per item: `> 📭 **Unclaimed request** — a `{action}` addressed to `{to}` has sat in the inbox for {age} and **no surface ever claimed it**. Nothing retired it, so nothing would ever have told you. The work did not happen.` Then check whether a surface is even alive (`~/.aios/surfaces/*.json` naming a **running pid**) before offering to re-drop: if none is, re-dropping writes another file nobody will read — hand the operator the `spawn <name> "<task>"` line instead. If one *is* alive, offer to re-drop or discard, ask once, and never auto-re-drop (a stale request's instructions may no longer apply). A `.holding` file is **not** this: that is a claimed request legitimately waiting, and it is never reported here.

**Then REMOVE the file — a dead letter is not self-clearing.** `.undelivered` files are inert: nothing consumes them, so one that has been handled keeps surfacing on *every* future `/today` and `/close-day`. Once the operator decides, act on it in the same turn:
- **Re-drop** → write a fresh request to `~/.aios/spawn-inbox/<name>.json` (a fulfiller only picks up `.json`, never `.undelivered`), **then** `rm` the dead letter — it's a spent artifact once its content has been re-sent.
- **Discard** → `rm` it.
- **Keep as evidence** → move it out of the inbox (e.g. under `vault/00 - notes/logs/`), don't leave it in place.

Never leave a handled dead letter sitting there. A permanent alert is how an operator learns to ignore the check — which would recreate the exact blindness this whole mechanism exists to remove.

**Why this is in the morning batch:** a dead letter is written honestly and read by nobody, so the sender believes the handoff landed. Every hour it sits is an hour of work that silently didn't happen — and the operator is the only party who can tell whether it still matters.
...
**Backup warning rendering (handled in synthesis, Message 2):**

When the vault backup check returns `backup: none` → one line at the top of the **Rhythm** section, a warning rather than a task:

> *"⚠️ **Your vault is only on this computer** — nothing is backed up yet. When you have a GitHub account: `! gh auth login`, then say *connect my vault to GitHub* (~2 min)."*

If the operator says it, run SETUP §8 (`gh repo create {username}/aios --private --source=. --push`). It re-surfaces every morning until then, deliberately: this is a data-loss risk, not an invitation.

When it returns `backup: framework-origin ({remote})` → same line, but say what is different: *"⚠️ Your vault pushes to `{remote}`, which is the public AIOS framework repo, not a private one — a push would try to publish your notes there."* Offer SETUP §8 now, or `git -C ~/aios remote remove {remote}` — **the remote the probe named, which is not always `origin`** — to make it local-only until then.

**Week-1 check-in rendering (handled in synthesis, Message 2):**

When the Week-1 probe returns `pending-setup: day7-checkin due=…` → add one line to the **Rhythm** section, and keep it small. This is a nudge the operator was promised, not a task they signed up for:

> *"🎓 **Week 1 with the AIOS.** Worth 10 minutes: `/agent onboarding-aios` reviews what's actually been useful, what you haven't touched yet, and whether anything from the deeper setup is worth doing now. Say the word, or ignore this — it won't ask twice."*

Delete the marker once the check-in runs, **or** once the operator declines it: `Bash(rm -f ~/aios/vault/.pending-day7-checkin)`. A nudge that reappears after being declined stops being a nudge, which is the failure mode the quota marker deliberately accepts (it re-surfaces until completed) and this one deliberately does not — the quota capture is unfinished *setup*, while this is an *invitation*.
...
- **Observed-context staleness check (streak-independent backstop) — run the hook, do not compute it by hand:**
  ...
  ```bash
  UV_CACHE_DIR="${TMPDIR:-/tmp}/uv-cache" uv run ~/aios/hooks/observed-staleness.py
  ```
...
**Daily notes:**
- **Most recent daily note** (NOT hardcoded to yesterday's date): take the latest `YYYY-MM-DD.md` dated before today across **every** month folder under `01 - calendar/` — the same note the close-day precondition probe above already found, so reuse its answer rather than listing again. **Exclude** weekly plans (`W{N}-plan.md`, `W{N}-summary.md`) and any other non-daily files. Two months back is not "any gap": after a three-week holiday that spans a month boundary the last note is older than that, and its carries would be dropped. **Critical — this is where carry-forwards come from. If this file isn't found, carried items are silently dropped.**
...
- The `_index.md` already loaded in Message 1a is the check: does it have a `## Roadmap board` section (any heading containing "Roadmap board")? **Present** → that section is the source for `## The Board` in today's note; note its generation date for the staleness rule. **Absent** → one existence probe: `Grep` for `type: roadmap` in frontmatter across `00 - notes/` **excluding `00 - notes/logs/`**, and check any hit's `status:` isn't `archived`. **The exclusion is load-bearing, not tidiness** — snapshots are byte-for-byte copies, so every archived roadmap carries `type: roadmap` + `status: live` and answers this probe exactly like the real file. This is the path that builds the Board, so its wrong answer is the one the operator reads each morning (CLAUDE.md § Ship-time truth-flip, rule 1). **No live roadmap files → skip everything Board-related silently** (the zero-config path — project notes are the truth surface and the snapshots already carry them). Live files found but no board section yet (the vault graduated but `/close-day` hasn't run since) → read the live roadmap file(s) directly this once and derive the Board inline; tonight's close-day takes over the derivation.
...
- **Extract carry-forwards: ZERO TOLERANCE for dropped items.** Scan all unchecked `- [ ]` across ALL sections of the most-recent daily note — Rhythm, Parking lot, Horizon, Carries forward, AND the Close of Day `### Carries forward` subsection. **Exclude the `## Agents can handle` section** — it's a regenerated mirror (`- 🤖` lines, not `- [ ]` tasks), rebuilt fresh below, never a carry source. Dedup by core task name (ignore emojis, time slots, source tags); keep highest carry count + most actionable version. **Then drop any open line whose done twin is in the same note:** if a line with the same core identity (same dedup key) is `[x]` or `~~struck~~` anywhere in that note — any section, the `## Agents can handle` mirror included — the task is done and does not carry. One task is often written in several places (the morning plan, a session capture written before it shipped, the close-of-day list) and the done mark lands on only one copy; carrying the others re-opens finished work every morning, with a growing carry count that makes it look more urgent each day. This is the same cross-section identity match Glass already applies to the agents mirror. **A twin only counts as done when it closes the whole task:** an `[x]` that notes what is still open is partial work and does not count; for a multi-part task only a struck title counts, never struck sub-items alone; a recurring task (daily, per meeting, per time slot) is closed only for the instance that was marked; and an open line written after the done one as an explicit reopening stays open. When the match is uncertain (similar wording, a different deliverable, a different instance), carry it and say so rather than drop it. After writing today's note, verify per item (clean pass step 2 below): every unique unchecked item from the previous note is placed somewhere (Rhythm / Parking lot / Compass) or was removed on purpose. Carries that have been silent 3+ days still surface.
...
- **Target-aware carry display** (opt-in). Carries can optionally include a `target:` metadata field — the actual deadline, distinct from the carry's age. Format: `_(carried ×N, target: 2026-05-15)_` or `_(carried ×N, target: 2026-W19)_`. When rendering:
  ...
  - **`target:` absent** → display `_(carried ×N, no target)_` — and the count-based table above applies. That table exists for exactly these carries: untagged, no target, no window. A carry with a target is judged by its target (the two bullets above), a tagged one by its tag, a window-paced one by its window (below) — **never** by raw age.
...
- **Compass hiding** (>14 days). Long-horizon `waiting-on-date` carries should NOT clutter daily notes when their target is far out — they belong in project notes + `_index.md` snapshots only, re-entering the daily flow as the date approaches:
...
- **Post-write clean pass** (run silently before commit; fix issues, don't ask):
  1. **Dedup across sections.** Scan Rhythm + Parking + Horizon + secondary-machine results (if present) + Energy note for the same task appearing in more than one place. Match by core task name (ignore emojis, source tags, carry counts). If a task is in Rhythm AND Parking → keep only Rhythm. If a review item is in the secondary machine's results AND Parking → keep only the Rhythm/Evening placement.
  2. **Drop check (carries) — by identity, never by count.** For each unique unchecked `- [ ]` item of the previous note (same dedup key as the extraction), confirm it is placed in today's note, or that it left on purpose: **parked** (matched `## Explicitly NOT doing` — removed by design) or **done elsewhere** (a `[x]` or struck twin with the same identity in the previous note that closes the whole task, under the same limits as the extraction above — re-check them here; a twin that fails them is not an exit) or **compass-hidden** (target > 14 days — confirm it is in its project note's to-dos; if it is not, it is not hidden, it is lost: put it there or in Parking). Anything else missing → add to Parking before committing. Counting is not a drop check: A, B yesterday and A plus a new C today are two and two, and B is gone.
...
_{Why this matters or what it unblocks — one line. **If `growth.md` carries a `## Compass` with content under it, this line also names the value the deliverable expresses** — one clause, folded into the existing sentence, never a second line or a labelled field: "…and it's your teaching, wearing a suit." The point is that the day's biggest destination is visibly an expression of the compass rather than a free-standing spike. **No `## Compass` → write this line exactly as before and say nothing about purpose** — the offer to derive one belongs to `/close-day`, once, and only when the variation gate has been crossed (see the `finding-your-why` skill). Do not stretch for the link when it isn't there: some days ship maintenance, and forcing a value onto a chore is how the whole layer loses credibility.}_
...
**A third routine shape exists beside reading and writing, and it is documented HERE rather than in `USER.md`'s example block** — `USER.md` is Tier-2 and never syncs, so a convention living only there would reach fresh clones and no existing operator. **Becoming**: one voice-answerable question a day, drawn from whichever dimension of the operator's own context is **thinnest, computed from disk** (deficit-driven, never the next card in a fixed deck — that is what makes it a ratchet), appended to one file per month so capture costs no filing decision. Its metric is **the share, not the streak**: an activity count can be *fully satisfied while the thing it exists to grow keeps falling*, which is the failure mode this shape is built against. Why it needs a forcing function at all: every other layer of a vault grows as a byproduct of work — ships generate project state, sessions generate observed context, living generates daily notes — while the identity layer only grows by deliberate reflection, so it **falls every time anything else succeeds**. Reading feeds what the operator knows, writing feeds what they ship, this feeds **who they are**. Two constraints: every question asks for an **episode and its specifics** (what happened, who was there, what it cost) and never for feelings in the abstract — this framework is explicit that a vault is not a journal; and the andon is **two consecutive weeks of a falling share**, not a missed day. Method: the **`finding-your-why`** skill.
