...
- `Bash(p="$(git -C ~/aios rev-parse --absolute-git-dir 2>/dev/null)/aios-push-pending"; if [ -f "$p" ]; then echo "pending-push: YES (marked $(cat "$p"), $(git -C ~/aios rev-list --count @{u}..HEAD 2>/dev/null || echo '?') commit(s) unpushed)"; else echo "pending-push: none"; fi)` — **unpushed-commit check.** `aios-commit` leaves `.git/aios-push-pending` when a push fails. Only some causes self-heal: offline lifts on its own, but a sandboxed call has no network egress and every later sandboxed `aios-commit` fails identically — so the commit sits unpushed while the deferral message reads as handled. If `YES` → surface per § Pending-push rendering below.
...
**Pending-push rendering (handled in Message 1a, alongside the aios-update check):**

When the unpushed-commit check returns `pending-push: YES` → **retry the push immediately** (it needs NETWORK, so if your Bash calls are sandboxed, run this one with network access), then report the outcome in one line:

```bash
p="$(git -C ~/aios rev-parse --absolute-git-dir 2>/dev/null)/aios-push-pending"
if git -C ~/aios push; then rm -f "$p"; echo "pending-push: cleared"; else echo "pending-push: STILL blocked"; fi
```

- **Cleared** → one line, no callout: *"Pushed {N} commit(s) that were stranded from a previous session."* Don't make it a task; it's done.
- **Still blocked** → callout at the top of the plan: `> ⚠️ **{N} commit(s) committed but never pushed** — the retry failed again. Read the git error: blocked egress needs the git transport host (e.g. `github.com`, NOT `api.github.com`) in the sandbox network allowlist; auth needs an ssh key/agent.` This is worth surfacing at morning specifically because a vault that never pushes is invisible to every other machine and to any overnight queue generated from it.
