...
_{Why this matters or what it unblocks — one line. **If `growth.md` carries a `## Horizon`, this line also names the value the deliverable expresses** — one clause, folded into the existing sentence, never a second line or a labelled field: "…and it's your teaching, wearing a suit." The point is that the day's biggest destination is visibly an expression of the compass rather than a free-standing spike. **No `## Horizon` → write this line exactly as before and say nothing about purpose** — the offer to derive one belongs to `/close-day`, once, and only when the variation gate has been crossed (see the `finding-your-why` skill). Do not stretch for the link when it isn't there: some days ship maintenance, and forcing a value onto a chore is how the whole layer loses credibility.}_
