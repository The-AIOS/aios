...
When it returns `backup: framework-origin` → same line, but say what is different: *"⚠️ Your vault's `origin` is the public AIOS framework repo, not a private one — a push would try to publish your notes there."* Offer SETUP §8 now, or `git -C ~/aios remote remove origin` to make it local-only until then.
