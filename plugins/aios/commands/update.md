...
- **Skills:** every source folder under `skills/` — `skills/aios/` plus the vendored `skills/anthropic/`, `skills/superpowers/`, `skills/cloudflare/` — never `skills/custom/` or `skills/<company>/`. A newly vendored source arrives without being named here: Step 2 derives the layers from the clone.
...
> **A FORKED vault is the case this tier's justification does not cover — reported by an operator, 2026-08-13.** Tier 0 is defended with *"an operator's vault has no CI"*, which is true for a clean clone and **false for a fork** — and forking is a normal install path. A fork carries `.github/` and `tests/` from the moment it is created, but because `.github/` is Tier 0 it then **never updates again**: the fork's *frozen* workflow goes on judging the *new* content `/aios:update` legitimately delivers. Canonical had already widened an allowlist that no vault could ever receive, so the operator got red mail for healthy files — and pulling the new workflow by hand only moves the error, since it invokes suites written against canonical's tree. **Fix shipped in `validate.yml`, not here:** the ten canonical-contract jobs carry `if: github.event.repository.fork != true`, so they *skip* rather than fail in a fork, while the two operator-relevant guards (credentials scan, note frontmatter) keep running. A contributor PR is unaffected — `pull_request` events evaluate `github.event.repository` against the **base** repo. Same role-aware shape as `company-template`'s `personal_leak` job.
...
- **`hooks/install-git-hooks.sh` / `.ps1` updated, OR any `hooks/git/*` changed** → run the git-hooks installer, platform-guarded exactly like the wrapper installer (`bash $HOME/aios/hooks/install-git-hooks.sh` on macOS/Linux; on Windows `pwsh -File $HOME/aios/hooks/install-git-hooks.ps1`, falling back to `powershell -File …`). Idempotent. Report anything it says it normalized.
...
Skip layers without a `custom/` subfolder (no opportunity for duplicates there). **Bundled content for every layer lives under `{layer}/aios/` (+ source-peer bundles `anthropic/`, `superpowers/`, `cloudflare/` for skills); the layer root and `custom/` should contain only `_index.md` + genuine operator extensions after cleanup.**
...
1. Apply the replace for `update.md` immediately:
   - **Run the three-way compare first — do NOT back up unconditionally.** This step used to say *"backup local … (operator divergence preserved per the standard rule)"*, which reads like deference to § Backup-on-divergence and is the opposite of it: that rule's entire first row is **identical to baseline → overwrite silently, no backup**, because a stale file is not a personalization. An unconditional copy here therefore manufactures exactly the noise the three-way compare exists to prevent — and it fires on *every* sync where `update.md` changed, i.e. often, in a directory the operator is supposed to be able to scan for real losses. Measured 2026-09-13 on a live vault: the backed-up file hashed **identical to the baseline**, so the sync's only backup that day carried nothing the operator had written. **Apply the same three outcomes as Step 3.2** — identical-to-baseline overwrites silently, local-AHEAD is kept, both-moved backs up. There is nothing special about this file except that it is applied earlier.
...
5. **Continue this run under the new spec** (do not exit expecting an inner run to do the work). If for any reason the new spec cannot be read, finish the sync under the current spec and tell the operator to re-run `/aios:update` in a **fresh session** — **start a new Claude session** (`/exit` then `claude` again, or *Close the Session* and a new tab in the AIOS App). **Only the session** — the app, the terminal window and the editor can all stay open, and sessions already running keep working — to pick up the new logic — the update still lands either way.

Report to operator at the end: *"`/aios:update` self-updated and continued under the new spec — {summary of what this run then processed}."* No operator action required. **Do not describe an "inner run" or a hand-off** — there is neither; one run applied the new spec and kept going. Wording that implies a second run makes a stalled sync look like a completed one, since the reader assumes something else finished the job.
...
#
# Not hypothetical — it happened. A failed clone, a partial clone, or a cleanup that
# ran too early all present identically to a clean vault. Assert, then compare.
#
# ⚠️ Do NOT "modernize" the literal `/tmp` above into `${TMPDIR:-/tmp}`. That looks
# more correct and is strictly worse here: `$TMPDIR` is **not stable across tool
# calls** in a sandboxed harness (a sandboxed call resolves a different $TMPDIR than
# an un-sandboxed one), so the clone written in Step 1 becomes invisible in this step
# while the path expression still reads fine — which is exactly how this reconcile
# once reported "0 drift" having compared nothing. `/tmp` is the right choice
# precisely because it is boring and fixed: writable when Step 1 clones (that call is
# un-sandboxed anyway, since cloning needs network) and readable from any later call,
# sandboxed or not.
if [ ! -d "$CLONE/plugins" ] || [ ! -d "$CLONE/agents" ]; then
  echo "FATAL: clone not found at $CLONE — the reconcile cannot run." >&2
  echo "Re-clone (Step 1), then re-run the reconcile." >&2
...
- **Auto-apply, never ask — but never DOWNGRADE.** Tier 1 changes are mandatory infra and the command presents no diff approval flow. That contract assumes the vault is *downstream*, which is true for a consumer and false for anyone whose copy is **ahead** — the framework author, and any fork operator who improves a command before canonical receives it. For them a Tier-1 file that "differs" is often newer, and overwriting it is a silent downgrade that reports success. So the three-way compare now asks a second question before overwriting: **did canonical actually change this file?** If it did not, the local copy is ahead and is kept. This adds a *classifier*, not an approval flow — auto-apply is preserved; it is simply stopped from being wrong in the one direction it cannot detect on its own. **Known limit, stated rather than implied:** when BOTH sides moved, this still backs up and takes canonical. Distinguishing a real merge there needs machinery (a decision ledger, a 3-way merge) whose own failure mode is *suppressing upstream changes* — the opposite of this command's job — so it is deliberately not built. _(Raised as an RFC 2026-08-18 by an operator running a deterministic classifier since 2026-06-23 across 135 adjudications; this takes the one branch that needs no new state and declines the ledger, on the strength of a flaw they reported in it themselves.)_ The operator sees a report of what was done, not a multiple-choice menu.
