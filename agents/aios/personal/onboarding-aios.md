...
- **Semantic triggers (route here automatically):**
  ...
  - **Teach-me / show-me (returning or re-orienting operator):** *"teach me to use AIOS"*, *"show me the new AIOS"*, *"walk me through what's new"*, *"what changed"*, *"what should I be using"*, *"give me a tour"*
...
- **Programmatic triggers** (other commands hand off to this agent on first contact):
  ...
  - **Post-major-update session** — when a session notices the vault just jumped a long way (a `/aios:update` that applied many entries at once), it MAY offer this agent once: *"That update covered a lot. Want me to spawn `onboarding-aios` to walk through what's new — or skip and discover as you go?"* Don't auto-fire; offer, and only once. (Superseded the one-time 2026-05 migration trigger, which keyed off a `pre-aios-migration-*` git tag no current operator can have.)
