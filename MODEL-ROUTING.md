...
>
>   **A Bash job still inherits the allow rules of BOTH layers it keeps.** `--setting-sources user,project` drops the local layer only. One operator's user settings allowed `Bash(touch *)`, and that alone let an off-allowlist `touch` run with auto-approve off; another operator's *project* settings carried `Bash(python3 ~/aios/hooks/:*)`, which lets a routine run any hook script outside its allowlist. Two ways out, and the trade is yours. **Narrow those rules** — but in your interactive settings that also means prompts where there were none. Or **give the routine its own folder** with its own `.claude/settings.json` (its allow list, its deny rules, the sandbox block above) and run it there with `--setting-sources project`: measured, a user-level `Bash(touch *)` no longer applied. Don't simply drop the vault's project layer for a routine that runs *in* the vault — it carries the deny rules and the PreToolUse guard that routine should keep. For a job that needs Bash, keep `--allowedTools` to what it runs and read both allow lists with the same eye. And read-only commands inside the working directory are auto-approved, so run such a job from a directory holding only what it needs.
>
