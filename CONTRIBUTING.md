...
**Some of it repairs itself.** `bash .github/scripts/autofix.sh` fixes the failures that have exactly one correct answer — today, the capability counts in `TOOLS.md`, `README.md` and `agents/_index.md`, derived from the folders rather than from a stored number. `--check` reports without writing. On `main` CI applies it and commits, so that class never reaches anyone's inbox; on a pull request the check still fails, because CI cannot (and must not) write to a fork's branch.

**It lives under `.github/`, and that is deliberate.** Everything in `.github/` and `tests/` is **canonical-only** — it serves CI and contributors, and `/aios:update` never syncs it into an operator's vault (`update.md` → Tier 0). Repo maintenance tooling belongs there for exactly that reason. `hooks/` is the opposite category: runtime code an operator's own sessions and commands invoke, which is why it *does* ship. **So put a new script where its audience is** — a contributor tool under `.github/scripts/`, a runtime tool under `hooks/` — and never in a new top-level folder, which would reach every vault on the next sync. A guard fails the build if canonical grows a top-level directory that is neither classified Tier-1 nor denylisted Tier-0.
