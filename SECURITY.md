...
**Claude Code's sandbox does not change that by default.** Sandboxed Bash can read these folders unless each one is named in the sandbox's `filesystem.denyRead`, and a `Read(...)` deny rule in `permissions` does not stop it. If you run jobs under the sandbox, list every folder above there: [`MODEL-ROUTING.md`](./MODEL-ROUTING.md) § Verify an id before you trust it has the settings.
