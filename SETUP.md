...
1. Confirm Prerequisites are installed — Obsidian + Node/Git/gh/Python/uv/Claude Code, **plus ONE execution surface**: the **AIOS App** *or* Antigravity IDE/VS Code with AIOS Glass. If the operator reached you from the AIOS App, that surface is already satisfied — do **not** send them to install an IDE or Glass. If anything else is missing, walk the OS-specific block from "Prerequisites" §
   ...
   **Before leaving step 1, ask for everything only the operator can do — in ONE message, now.** Two things in this sequence are out of your hands, and on a real Windows install through the App they surfaced as two separate stops: GitHub login halfway through step 2, and file access at step 8, after work had already been done around the block. Each stop is a moment the operator is handed a command they did not expect, mid-flow. Find both up front, ask for them together, and keep working while they do it:
   ...
   - **`needs: gh-login`** → *"Type `! gh auth login --hostname github.com --git-protocol https --web` — it opens your browser and signs you in to GitHub."* The flags answer the three questions a newcomer cannot (which host, which protocol, which method). App operators have usually done this in the App's own GitHub step, so this is normally silent on that path.
   ...
   - **Neither is needed until later**, so do not wait on them. The clone (step 2) needs no login, and the private repo can be created at the end of step 2 once `gh auth status` passes.
...
- **Sign in to GitHub** — type `! gh auth login --hostname github.com --git-protocol https --web` and approve in the browser. Needed for your private vault repo. (If you came through the AIOS App and did its GitHub step, you'll skip this.)
